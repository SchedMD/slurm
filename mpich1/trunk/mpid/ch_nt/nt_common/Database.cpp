...
bool Database::Init()
{
	...
	if (bFound)
	{
		...
		// connect to server
		ret_val = NT_Tcp_connect(sock, m_pszServerHost, m_nServerPort);
		if (ret_val)
			return (dbs_error("Database::Init: NT_Tcp_connect failed", ret_val) != 0);
...
int Database::Delete()
{
	...
	ret_val = NT_Tcp_connect(sock, m_pszServerHost, m_nServerPort);
	if (ret_val)
		{dbs_error("Database::Delete: NT_Tcp_connect failed", ret_val, sock, sock_event); return MPI_DBS_FAIL;}
...
int Database::Get(char *pszKey, void *pValue, int *length)
{
	...
	ret_val = NT_Tcp_connect(sock, m_pszServerHost, m_nServerPort);
	if (ret_val)
		{dbs_error("Database::Get: NT_Tcp_connect failed", ret_val, sock, sock_event); return MPI_DBS_FAIL;}
...
int Database::Put(char *pszKey, void *pValue, int length, bool bPersistent)
{
	...
	ret_val = NT_Tcp_connect(sock, m_pszServerHost, m_nServerPort);
	if (ret_val)
		{dbs_error("Database::Put: NT_Tcp_connect failed", ret_val, sock, sock_event); return MPI_DBS_FAIL;}
...
int Database::Print(char *pBuffer, int *length)
{
	...
	ret_val = NT_Tcp_connect(sock, m_pszServerHost, m_nServerPort);
	if (ret_val)
		{dbs_error("Database::Print: NT_Tcp_connect failed", ret_val, sock, sock_event); return MPI_DBS_FAIL;}
