     ...
     /* Time passing a message round a ring */
{
  ...
  if ( (msg = p4_shmalloc((unsigned) max_len)) == (char *) NULL)
      p4_error("Ring: failed to allocate msg",max_len);
  ...
  while (lenbuf <= max_len) {
    ...
    if (me == 0) {
      (void) p4_sendfc(type, left, buffer, lenbuf);
      (void) p4_recvfc(&type, &right, &msg, &msg_len);
    }
    else {
      (void) p4_recvfc(&type, &right, &msg, &msg_len);
      (void) p4_sendfc(type, left, msg, msg_len);
    }
    used = p4_clock() - start;
