...
{
	DWORD size = 100;
	TCHAR pszHost[100], pszCmdLine[MAX_PATH], pszEnv[256], pszExtra[MAX_PATH];
	STARTUPINFO saInfo;
	...
	else
	{
		...
		{
			_tprintf(TEXT("CreateFile failed: %d\n"), GetLastError());
			return;
		}
	...
	WaitForMultipleObjects(g_nHosts, hProcess, TRUE, INFINITE);
