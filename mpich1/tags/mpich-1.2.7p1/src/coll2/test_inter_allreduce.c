...
/*  Created: Mar.9, 2000, Last modified: Mar.9, 2000
 * -------------------------------------------------
 *           Test Program for inter_Allreduce()
 * -------------------------------------------------
 */
...
*/
	...
	/* Prepare the data (int, my_rank for now) to be contributed.
	 * Note that the max. value of my_rank at the other group is
	 * received by me.
	 */
	data = global_rank;
	...
	fprintf(fptr, "Data contributed     : %d\n", data);

	/* Now call the inter_Allreduce */
	startwtime = MPI_Wtime();	/* Start timer */
	mpi_errno = MPI_Allreduce(&data, &result, 1, MPI_INT, MPI_MAX, inter_comm);

	endwtime = MPI_Wtime();		/* Stop timer */
	fprintf(fptr, "WC Time taken for Allreduce = %f\n",
				(endwtime - startwtime)); 

	if(mpi_errno)
		fprintf(fptr, "\nERROR: during inter_Allreduce!\n");
	else
		fprintf(fptr, "\ninter_Allreduce executed successfully!\n");

	fprintf(fptr, "Result after Allreduce	: %d\n", result);
...
/* End of program: test_inter_allreduce.c */
