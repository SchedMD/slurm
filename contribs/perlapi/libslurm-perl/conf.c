...
slurm_ctl_conf_to_hv(slurm_ctl_conf_t* conf, HV* hv)
{
	...
	if(conf->acct_storage_loc)
		STORE_FIELD(hv, conf, acct_storage_loc, charp);
	if(conf->acct_storage_type)
		STORE_FIELD(hv, conf, acct_storage_type, charp);
	if(conf->acct_storage_user)
		STORE_FIELD(hv, conf, acct_storage_user, charp);
	if(conf->acct_storage_host)
		STORE_FIELD(hv, conf, acct_storage_host, charp);
	if(conf->acct_storage_pass)
		STORE_FIELD(hv, conf, acct_storage_pass, charp);
	STORE_FIELD(hv, conf, acct_storage_port, uint32_t);
