...
void parse_node_pointer(zval * sub_arr, node_info_t * node_arr)
{
	...
	zend_add_valid_assoc_time_string(sub_arr, "Boot Time", &node_arr->boot_time);
	...
	if(node_arr->reason!=NULL) {
		zend_add_valid_assoc_string(sub_arr, "Reason", node_arr->reason);
		zend_add_valid_assoc_time_string(sub_arr,"Reason Timestamp", &node_arr->reason_time);
		add_assoc_long(sub_arr, "Reason User Id", node_arr->reason_uid);
	} else {
		add_assoc_null(sub_arr, "Reason");
		add_assoc_null(sub_arr, "Reason Timestamp");
		add_assoc_null(sub_arr, "Reason User Id");
	}
	zend_add_valid_assoc_time_string(sub_arr, "Slurmd Startup Time", &node_arr->slurmd_start_time);
	...
	while (rslt != NULL) {
		if (i == 0) {
			...
			if(strcmp(rslt,"(null)")==0) {
				...
				zend_add_valid_assoc_string(result_arr, tmp, rslt);
			}
		}
		i++;
		if (i == 2) {
...
PHP_FUNCTION(slurm_slurmd_status)
{
	...
	array_init(return_value);
	zend_add_valid_assoc_time_string(return_value,"Booted_at",&status_ptr->booted);
	zend_add_valid_assoc_time_string(return_value,"Last_Msg",&status_ptr->last_slurmctld_msg);
	...
	add_assoc_long(return_value,"Actual_Sockets",status_ptr->actual_sockets);
	add_assoc_long(return_value,"Actual_Cores",status_ptr->actual_cores);
	add_assoc_long(return_value,"Actual_Threads",status_ptr->actual_threads);
	add_assoc_long(return_value,"Actual_Real_Mem",status_ptr->actual_real_mem);
	add_assoc_long(return_value,"Actual_Tmp_Disk",status_ptr->actual_tmp_disk);
	add_assoc_long(return_value,"PID",status_ptr->pid);
	zend_add_valid_assoc_string(return_value, "Hostname", status_ptr->hostname);
	zend_add_valid_assoc_string(return_value, "Slurm Logfile", status_ptr->slurmd_logfile);
	zend_add_valid_assoc_string(return_value, "Step List", status_ptr->step_list);
	zend_add_valid_assoc_string(return_value, "Version", status_ptr->version);
}
...
PHP_FUNCTION(slurm_version)
{
	...
	default:
		array_init(return_value);
		add_next_index_long(return_value, SLURM_VERSION_MAJOR(SLURM_VERSION_NUMBER));
		add_next_index_long(return_value, SLURM_VERSION_MINOR(SLURM_VERSION_NUMBER));
		add_next_index_long(return_value, SLURM_VERSION_MICRO(SLURM_VERSION_NUMBER));
		break;
	}
}
...
PHP_FUNCTION(slurm_hostlist_to_array) {
	...
	if(zend_parse_parameters(ZEND_NUM_ARGS()TSRMLS_CC, "s|d", &host_list, &lngth) == FAILURE) {
		RETURN_LONG(-3);
	}
...
PHP_FUNCTION(slurm_array_to_hostlist) {
	...
	if(zend_parse_parameters(ZEND_NUM_ARGS()TSRMLS_CC, "a", &node_arr) == FAILURE) {
		RETURN_LONG(-3);
	}
...
PHP_FUNCTION(slurm_print_partition_names)
{
	...
	array_init(return_value);
	for (i = 0; i < prt_ptr->record_count; i++) {
		add_next_index_string(return_value, prt_ptr->partition_array->name, 1);
		prt_ptr->partition_array++;
	}
}
...
PHP_FUNCTION(slurm_get_specific_partition_info)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS()TSRMLS_CC, "s|d", &name, &lngth) == FAILURE) {
		RETURN_LONG(-3);
	}
...
PHP_FUNCTION(slurm_get_partition_node_names)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS()TSRMLS_CC, "s|d", &prt_name, &lngth) == FAILURE) {
		RETURN_LONG(-3);
	}
...
PHP_FUNCTION(slurm_get_node_names)
{
	...
	if (node_ptr->record_count != 0) {
		...
		for (i = 0; i < node_ptr->record_count; i++) {
			add_next_index_string(return_value, node_ptr->node_array->name, 1);
			node_ptr->node_array++;
		}
	} else {
		RETURN_LONG(-1);
...
PHP_FUNCTION(slurm_get_node_elements)
{
	...
	for (i = 0; i < node_ptr->record_count; i++) {
		...
		add_assoc_zval(return_value, node_ptr->node_array->name, sub_arr);
		node_ptr->node_array++;
	}
}
...
PHP_FUNCTION(slurm_get_node_element_by_name)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS()TSRMLS_CC, "s|d", &node_name, &lngth) == FAILURE) {
		RETURN_LONG(-3);
	}
...
PHP_FUNCTION(slurm_get_node_states)
{
	...
	array_init(return_value);
	for (i = 0; i < node_ptr->record_count; i++) {
		add_next_index_long(return_value,node_ptr->node_array->node_state);
		node_ptr->node_array++;
	}
}
...
PHP_FUNCTION(slurm_load_job_information)
{
	...
	for (i = 0; i < job_ptr->record_count; i++) {
		...
		ALLOC_INIT_ZVAL(sub_arr);
		array_init(sub_arr);
		parse_assoc_array(slurm_sprint_job_info(job_ptr->job_array, 1), "= ", sub_arr);
...
PHP_FUNCTION(slurm_load_partition_jobs)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS()TSRMLS_CC, "s|d", &pname, &lngth) == FAILURE) {
		RETURN_LONG(-3);
	}
