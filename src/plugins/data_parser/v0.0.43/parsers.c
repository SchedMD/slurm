...
#define PARSER_ARRAY(type) _v43_parser_array_##type
#define PARSER_FLAG_ARRAY(type) _v43_parser_flag_array_##type
#define PARSE_FUNC(type) _v43_parse_##type
#define DUMP_FUNC(type) _v43_dump_##type
#define NEW_FUNC(type) _v43_openapi_new_##type
#define FREE_FUNC(type) _v43_openapi_free_##type
				...
				data_t *dst, args_t *args, data_t *parent_path)
{
	...
	on_warn(PARSING, parser->type, args,
		...
		"data_parser/v0.0.43 does not support parser %u for parsing. Output may be incomplete.",
		parser->type);
...
static const parser_t PARSER_ARRAY(ASSOC_REC_SET)[] = {
	...
	add_parse(TRES_STR, grp_tres_mins, "grptresmins", "GrpTRESMins - Total number of TRES minutes that can possibly be used by past, present, and future jobs in this association and its children"),
	...
	add_parse(UINT32_NO_VAL, max_submit_jobs, "maxsubmitjobs", "MaxSubmitJobs - Maximum number of jobs which can be in a pending or running state at any time in this association"),
	...
	add_parse(UINT32_NO_VAL, max_wall_pj, "maxwalldurationperjob", "MaxWallDurationPerJob - Maximum wall clock time each job can use in this association"),
...
static const parser_t PARSER_ARRAY(ASSOC)[] = {
	...
	add_skip(grp_tres_ctld),
	add_parse(TRES_STR, grp_tres_mins, "max/tres/group/minutes", "GrpTRESMins - Total number of TRES minutes that can possibly be used by past, present, and future jobs in this association and its children"),
	add_skip(grp_tres_mins_ctld),
	...
	add_skip(max_tres_pn_ctld),
	add_parse(UINT32_NO_VAL, max_wall_pj, "max/jobs/per/wall_clock", "MaxWallDurationPerJob - Maximum wall clock time each job can use in this association"),
...
static const parser_t PARSER_ARRAY(QOS)[] = {
	...
	add_parse(TRES_STR, grp_tres_mins, "limits/max/tres/minutes/total", "GrpTRESMins - Total number of TRES minutes that can possibly be used by past, present, and future jobs"),
	...
	add_parse(UINT32_NO_VAL, max_wall_pj, "limits/max/wall_clock/per/job", "MaxWallDurationPerJob - Maximum wall clock time each job can use"),
	...
	add_parse(TRES_STR, min_tres_pj, "limits/min/tres/per/job", "MinTRES - Minimum number of TRES each job running under this QOS must request"),
...
static const parser_t PARSER_ARRAY(SINFO_DATA)[] = {
	...
	add_parse(STRING, cpu_spec_list, "resource_spec/cpus", "CpuSpecList"),
	add_parse(UINT64, mem_spec_limit, "resource_spec/memory", "MemSpecLimit"),
