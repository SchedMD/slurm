...
static licenses_t *_find_license_in_list(list_t *license_list, uint16_t lic_id)
{
	return list_find_first(license_list, _license_find_rec_by_id, &lic_id);
}
...
static int _is_job_relevant(void *x, void *key)
{
	...
	if (running_job_ptr->license_list && args->needed_licenses) {
		for (uint32_t i = 0; i < args->license_cnt; i++) {
			...
			match = _find_license_in_list(running_job_ptr
							      ->license_list,
						      needed_license.lic_id);
			if (!match)
				continue;
...
static int _set_license_req(void *x, void *arg)
{
	...
	if ((match = _find_license_in_list(args->job_license_list,
					   future_license->lic_id))) {
		args->needed_licenses[args->license_cnt].lic_id =
			future_license->lic_id;
