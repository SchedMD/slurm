...
extern bool
proctrack_p_has_pid(uint64_t cont_id, pid_t pid)
{
	int local_pid = (int) pid;
...
proctrack_p_get_pids(uint64_t cont_id, pid_t **pids, int *npids)
{
	...
	if (sizeof(int32_t) == sizeof(pid_t)) {
		debug3("proctrack_p_get_pids: No need to copy "
		       "pids array");
		*npids = np;
		*pids = (pid_t *)p;
	} else {
		...
		debug3("proctrack_p_get_pids: Must copy pids array");
...
proctrack_p_wait(uint64_t cont_id)
{
	...
	while (proctrack_job_unreg(&jobid) != 0) {
		...
		if (delay < 120) {
			...
			proctrack_p_get_pids(cont_id, &pids, &npids);
			if (npids > 0) {
				for (i = 0; i < npids; i++) {
