...
static void _stop_stage_in(uint32_t job_id)
{
	...
	tok = strrchr(bb_config.stop_stage_in, '/');
	if (tok)
		xstrfmtcat(script_argv[0], "%s", tok + 1);
	else
		xstrfmtcat(script_argv[0], "%s", bb_config.stop_stage_in);
	xstrfmtcat(script_argv[1], "%s", "stop_stage_in");
	xstrfmtcat(script_argv[2], "%u", job_id);

	resp = _run_script("StopStageIn", bb_config.stop_stage_in, script_argv,
			   -1);
	if (resp) {
...
static void _stop_stage_out(uint32_t job_id)
{
	...
	script_argv = xmalloc(sizeof(char *) * 4);
	tok = strrchr(bb_config.stop_stage_out, '/');
	if (tok)
		xstrfmtcat(script_argv[0], "%s", tok + 1);
	else
		xstrfmtcat(script_argv[0], "%s", bb_config.stop_stage_out);
	xstrfmtcat(script_argv[1], "%s", "stop_stage_out");
	xstrfmtcat(script_argv[2], "%u", job_id);

	resp = _run_script("StopStageOut", bb_config.stop_stage_out,
			   script_argv, -1);
	if (resp) {
	...
	*dest = NULL;
	...
	if (bb_ptr->state != state) {
		...
		if ((bb_ptr->state == BB_STATE_STAGED_IN) &&
		    bb_config.prio_boost_alloc && job_ptr && job_ptr->details) {
			new_nice = (NICE_OFFSET - bb_config.prio_boost_alloc);
...
static void _load_state(uint32_t job_id)
{
	...
	tok = strrchr(bb_config.get_sys_state, '/');
	if (tok)
		script_args[0] = tok + 1;
	else
		script_args[0] = bb_config.get_sys_state;
	if (job_id) {
	...
	START_TIMER;
	resp = _run_script("GetSysState", bb_config.get_sys_state, script_args,
			   10);
	if (resp == NULL)
		return;
	END_TIMER;
	...
	if (s_p_get_string(&tmp, "TotalSize", state_hashtbl)) {
		total_space = bb_get_size_num(tmp);
		xfree(tmp);
		if (bb_config.debug_flag && (total_space != last_total_space))
...
static void _alloc_job_bb(struct job_record *job_ptr, uint32_t bb_size)
{
	...
	if (bb_config.prio_boost_use && job_ptr && job_ptr->details) {
		uint16_t new_nice = (NICE_OFFSET - bb_config.prio_boost_use);
		if (new_nice < job_ptr->details->nice) {
			int64_t new_prio = job_ptr->priority;
			new_prio += job_ptr->details->nice;
			new_prio -= new_nice;
	...
	script_argv = _build_stage_args(bb_config.start_stage_in,
					"start_stage_in", job_ptr, bb_size);
	if (script_argv) {
		bb_ptr->state = BB_STATE_STAGING_IN;
		bb_ptr->state_time = time(NULL);
		resp = _run_script("StartStageIn", bb_config.start_stage_in,
				   script_argv, -1);
		if (resp) {
...
//FIXME: How to handle various job terminate states (e.g. requeue, failure), user script controlled?
	...
	if (!bb_ptr) {
		...
		script_argv = _build_stage_args(bb_config.start_stage_out,
...
extern int bb_p_job_cancel(struct job_record *job_ptr)
{
	...
	if (!bb_ptr) {
		...
		script_argv = _build_stage_args(bb_config.stop_stage_out,
						"stop_stage_out", job_ptr, 0);
		if (script_argv) {
			bb_ptr->state = BB_STATE_STAGED_OUT;
			bb_ptr->state_time = time(NULL);
			resp = _run_script("StopStageOut",
					   bb_config.stop_stage_out,
					   script_argv, -1);
			if (resp) {
				error("%s: StopStageOut: %s", __func__, resp);
				xfree(resp);
