...
static void _rm_qos_tres_run_secs(job_record_t *job_ptr,
				  slurmdb_qos_rec_t *qos_ptr,
				  uint64_t *unused_tres_run_secs)
{
	slurmdb_used_limits_t *used_limits_a;
	slurmdb_used_limits_t *used_limits_u;
	int i;
	...
	used_limits_a = acct_policy_get_acct_used_limits(
		&qos_ptr->usage->acct_limit_list, job_ptr->assoc_ptr->acct);
	used_limits_u = acct_policy_get_user_used_limits(
		&qos_ptr->usage->user_limit_list, job_ptr->user_id);
	...
	for (i = 0; i < slurmctld_tres_cnt; i++) {
		...
		if (unused_tres_run_secs[i] > used_limits_a->tres_run_secs[i]) {
		...
		if (unused_tres_run_secs[i] > used_limits_u->tres_run_secs[i]) {
...
extern void priority_p_job_end(job_record_t *job_ptr)
{
	...
	_rm_qos_tres_run_secs(job_ptr, job_ptr->qos_ptr, unused_tres_run_secs);
	/*
	 * acct_policy_job_begin() also books the partition QOS when it is
	 * distinct from the job QOS. multifactor already gives that usage
	 * back; do the same here.
	 */
	if (job_ptr->part_ptr &&
	    (job_ptr->part_ptr->qos_ptr != job_ptr->qos_ptr))
		_rm_qos_tres_run_secs(job_ptr, job_ptr->part_ptr->qos_ptr,
				      unused_tres_run_secs);
