				  ...
				  step_record_t *step_ptr)
{
	...
	/*
	 * Locate the step row with sluid + (id_step, step_het_comp) rather than
	 * job_db_inx directly. The sluid is stable across resizes while
	 * job_db_inx is regenerated by job_record_set_sluid() on every resize,
	 * so the caller (stepmgr or slurmctld) may carry an old job_db_inx
	 * which would not match against the current job.
	 *
	 * The time_end=0 filter restricts to the still-running row of this
	 * step.
	 *
	...
	xstrfmtcat(query,
		   ...
		   mysql_conn->cluster_name, job_table, step_ptr->step_id.sluid,
		   step_ptr->step_id.step_id, step_ptr->step_id.step_het_comp);
	DB_DEBUG(DB_STEP, mysql_conn->conn, "query\n%s", query);
	rc = mysql_db_query(mysql_conn, query);
	xfree(query);
