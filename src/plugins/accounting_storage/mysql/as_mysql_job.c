				  ...
				  step_record_t *step_ptr)
{
	 ...
	 */
	if (step_ptr->step_id.sluid) {
		/*
		 * We need the subquery as the sluid does not change across
		 * resizes, but the job_db_inx does. stepmgr or slurmctld may
		 * carry an old job_db_inx which would not match against the
		 * current job.
		 *
		 * The "or (sluid=0 and job_db_inx=%"PRIu64")" covers jobs that
		 * were inserted by a <= 25.11 slurmdbd and continue running
		 * after the upgrade. Remove once 25.11 is no longer supported.
		 */
		xstrfmtcat(
			query,
			" where job_db_inx in ("
			"  select job_db_inx from \"%s_%s\" where sluid=%" PRIu64
			"  or (sluid=0 and job_db_inx=%" PRIu64 ")"
			" ) and id_step=%d and step_het_comp=%u and time_end=0",
			mysql_conn->cluster_name, job_table,
			step_ptr->step_id.sluid, step_ptr->step_id.sluid,
			step_ptr->step_id.step_id,
			step_ptr->step_id.step_het_comp);
	} else {
		/*
		 * Slurm <= 25.05 does not pack job_ptr->step_id.sluid. Fall
		 * back to the legacy job_db_inx lookup. Remove this once 25.05
