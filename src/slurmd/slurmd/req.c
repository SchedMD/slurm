...
_rpc_launch_tasks(slurm_msg_t *msg)
{
	...
	first_job_run = !slurm_cred_jobid_cached(req->step_id.job_id);
	...
	if (first_job_run) {
		...
		slurm_cred_insert_jobid(req->step_id.job_id);
...
static void _rpc_prolog(slurm_msg_t *msg)
{
	...
	if (first_job_run) {
		...
		slurm_cred_insert_jobid(req->job_id);
...
_rpc_abort_job(slurm_msg_t *msg)
{
	 ...
	 *   for this job.
	 */
	if (slurm_cred_begin_expiration(req->step_id.job_id) < 0) {
					   ...
					   bool *first_job_run)
{
	...
	while (*first_job_run) {
		...
		slurm_cond_timedwait(&conf->prolog_running_cond,
				     &prolog_mutex, &ts);
		*first_job_run = !slurm_cred_jobid_cached(job_id);
	}
