...
static bool _match_node_data(sinfo_data_t *sinfo_ptr, node_info_t *node_ptr)
{
	...
	if (params.match_flags.port_flag &&
	    (node_ptr->port    != sinfo_ptr->min_port))
		return false;
			  ...
			  uint32_t node_scaling)
{
	...
	if (sinfo_ptr->nodes_total == 0) {	/* first node added */
		...
		sinfo_ptr->min_port   = node_ptr->port;
		sinfo_ptr->max_port   = node_ptr->port;
		...
		if (sinfo_ptr->min_port > node_ptr->port)
			sinfo_ptr->min_port = node_ptr->port;
		if (sinfo_ptr->max_port < node_ptr->port)
			sinfo_ptr->max_port = node_ptr->port;
