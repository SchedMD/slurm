...
#define _printf_error(format, args...)           \
	if (quiet_flag != 1) {                   \
...
void _process_power_command(const char *tag, int argc, char **argv)
{
	...
	if ((argc <= max_argv) && (argc >= min_argv)) {
		...
		exit_code = 1;
		_printf_error("too few arguments for keyword:%s\n", argv[0]);
...
static void _setdebug(int argc, char **argv)
{
	...
	if (level == -1) {
		...
		if (*endptr != '\0' || level > 9) {
			exit_code = 1;
			_printf_error("invalid debug level: %s\n", argv[1]);
			return;
		}
	}
	...
	if (argc == 2) {
		...
		if (xstrncasecmp(argv[2], "Nodes=", 6)) {
			exit_code = 1;
			_printf_error("Invalid option: %s\n", argv[1]);
			return;
		}
...
static void _setdebugflags(int argc, char **argv)
{
	...
	if ((i == (argc - 1)) && (!xstrncasecmp(argv[i], "node=", 5) ||
		...
		exit_code = 1;
		_printf_error("invalid debug flag: %s\n", argv[i]);
...
static void _show_it(int argc, char **argv)
{
	...
	if (xstrncasecmp(tag, "aliases", MAX(tag_len, 1)) == 0) {
		...
		exit_code = 1;
		_printf_error("invalid entity:%s for keyword:%s \n",
			      tag, argv[0]);
	}
