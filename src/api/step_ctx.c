 ...
 *	node.
 * IN ctx - job step context generated by slurm_step_ctx_create
 * IN addr_array - address of nodes to run on
 * IN node_cnt - number of nodes to run on
 * RET SLURM_SUCCESS or SLURM_ERROR (with slurm_errno set)
 */
extern int
slurm_step_ctx_daemon_per_node_hack(
	slurm_step_ctx_t *ctx, char **addr_array, uint32_t node_cnt)
{
	...
	ctx->step_req->num_tasks = node_cnt;
	...
	ctx->step_req->num_tasks = new_layout->task_cnt = new_layout->node_cnt
		= node_cnt;

	for (i=0; i<node_cnt; i++) {
		name = slurm_conf_get_nodename_from_addr(addr_array[i]);
		if (!name) {
			if (hl)
				hostlist_destroy(hl);
			return SLURM_ERROR;
