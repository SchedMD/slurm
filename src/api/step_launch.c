 ...
 **********************************************************************/
static int  _decr_launch_count(void);
static void _set_launch_count(int pack_step_cnt);
...
static int launch_count = -1;
static pthread_mutex_t launch_count_lock = PTHREAD_MUTEX_INITIALIZER;
...
/*
 * Increment the count of separate steps launches (for pack jobs)
 */
static void _set_launch_count(int pack_step_cnt)
{
	slurm_mutex_lock(&launch_count_lock);
	if (launch_count == -1)
		launch_count = MAX(1, pack_step_cnt);
	slurm_mutex_unlock(&launch_count_lock);
}

/*
 * Decrement the count of separate and incomplete steps launches (for pack jobs)
 * Return the count of remaining steps to complete
 */
static int _decr_launch_count(void)
{
	int cnt;
			     ...
			     int pack_job_cnt)
{
	...
	_set_launch_count(pack_job_cnt);
...
void slurm_step_launch_wait_finish(slurm_step_ctx_t *ctx)
{
	...
	if (_decr_launch_count() > 0) {	/* More pack job steps */
		slurm_mutex_unlock(&sls->lock);
		 return;
	}
