 ...
 * PREEMPT_ENABLED - Set 'data' to a boolean if preemption is enabled or not.
 * PREEMPT_MODE - Set 'data' to uint16_t on which PreemptMode which should apply
 *                to stop this job.
 * PREEMPT_PREEMPTEE_LIST - Set 'data' to a list of pointers to preemptable
 *                          jobs. The jobs will be sorted in order from most
 *                          desirable to least desirable to preempt.
 * PREEMPT_GRACE_TIME - Set 'data' to a uint32_t of what grace_time should be
 *                      for the given job.
 */
