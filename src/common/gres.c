...
extern int _node_config_validate(char *node_name, uint32_t gres_cnt,
				 ...
				 char **reason_down, char *gres_name)
{
	int rc = SLURM_SUCCESS, colon_name_len;
	gres_node_state_t *gres_ptr;
	char *node_gres_config, *tok, *last = NULL;
	char name_colon[128];
	...
	colon_name_len = snprintf(name_colon, sizeof(name_colon), "%s:",
				  gres_name);
	if ((orig_config == NULL) || (orig_config[0] == '\0'))
		gres_ptr->gres_cnt_config = 0;
	else if (gres_ptr->gres_cnt_config == NO_VAL) {
		node_gres_config = xstrdup(orig_config);
		tok = strtok_r(node_gres_config, ",", &last);
		while (tok) {
			if (!strcmp(tok, gres_name)) {
			...
			if (!strncmp(tok, name_colon, colon_name_len)) {
				gres_config_cnt = strtol(tok+colon_name_len,
							 &last, 10);
				if (last[0] == '\0')
					;
				else if ((last[0] == 'k') || (last[0] == 'K'))
	...
	if ((fast_schedule < 2) && 
		...
		if (reason_down && (*reason_down == NULL)) {
			xstrfmtcat(*reason_down, "gres/%s count too low",
				   gres_name);
		}
		rc = EINVAL;
		...
		while (tok) {
			...
			if (strcmp(tok, gres_name) &&
			    strncmp(tok, name_colon, colon_name_len)) {
				xstrcat(new_configured_res, tok);
			} else {
				xstrfmtcat(new_configured_res, "%s:%u",
					   gres_name, gres_ptr->gres_cnt_found);
			}
			tok = strtok_r(NULL, ",", &last);
		}
		xfree(node_gres_config);
