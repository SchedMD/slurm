			  ...
			  List qos_list)
{
	...
	if (assoc_ptr->max_resv_jobs == INFINITE)
		debug2("  MaxResvJobs          : NONE");
	else if (assoc_ptr->max_resv_jobs != NO_VAL)
		debug2("  MaxResvJobs          : %u", assoc_ptr->max_resv_jobs);
					  ...
					  slurmdb_assoc_rec_t *in)
{
	...
	out->max_resv_jobs = in->max_resv_jobs;
