...
static void xtree_free_childs(xtree_t* tree, xtree_node_t* node)
{
        ...
        while (current_node) {
                ...
                if(tree->free) tree->free(free_later->data);
                xfree(free_later);
                --tree->count;
        }
}
...
void xtree_free(xtree_t* tree)
{
        ...
        if(tree->free) tree->free(tree->root->data);
        xfree(tree->root);
...
{
        if (!node || !tree || !tree->root) return NULL;
        return node->parent;
}

uint32_t xtree_get_count(xtree_t* tree)
{
    if (!tree) return UINT32_MAX;
    return tree->count;
}
                 ...
                 void* arg)
{
        ...
        while (current_node) {
                ...
                if (current_node->next) {
                        if (level >= min_level && !action(current_node->parent,
                                    XTREE_INORDER,
                                    level - 1,
                                    arg)) {
                                return current_node;
...
xtree_node_t* xtree_delete(xtree_t* tree, xtree_node_t* node)
{
        ...
        if (tree->free) tree->free(node->data);
        xfree(node);
        --tree->count;
                ...
                uint32_t* size)
{
        ...
        if (!tree || !tree->root || !node || !size) return NULL;
