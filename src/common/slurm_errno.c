...
slurm_errtab_t slurm_errtab[] = {
	...
	{ ERRTAB_ENTRY(ESLURM_TASKDIST_ARBITRARY_UNSUPPORTED),
	  "Current SwitchType does not permit arbitrary task distribution"},
	{ ERRTAB_ENTRY(ESLURM_TASKDIST_REQUIRES_OVERCOMMIT),
	...
	{ ERRTAB_ENTRY(ESLURM_INVALID_BLOCK_STATE),
	  "Invalid block state specified"			},
	{ ERRTAB_ENTRY(ESLURM_INVALID_BLOCK_LAYOUT),
	  "Functionality not available with current block layout mode"},
	{ ERRTAB_ENTRY(ESLURM_INVALID_BLOCK_NAME),
	...
	/* slurmd error codes */
	{ ERRTAB_ENTRY(ESLURMD_PIPE_ERROR_ON_TASK_SPAWN),
	  "Pipe error on task spawn"				},
	{ ERRTAB_ENTRY(ESLURMD_KILL_TASK_FAILED),
	  "Kill task failed"					},
	{ ERRTAB_ENTRY(ESLURMD_UID_NOT_FOUND),
	  "User not found on host"                              },
	{ ERRTAB_ENTRY(ESLURMD_GID_NOT_FOUND),
	...
	{ ERRTAB_ENTRY(ESLURMD_MODIFY_BATCH_DIR_ERROR),
	  "Slurmd could not chown or chmod a batch directory"	},
	{ ERRTAB_ENTRY(ESLURMD_CREATE_BATCH_SCRIPT_ERROR),
	  "Slurmd could not create a batch script"		},
	{ ERRTAB_ENTRY(ESLURMD_MODIFY_BATCH_SCRIPT_ERROR),
	...
	{ ERRTAB_ENTRY(ESLURMD_EPILOG_FAILED),
	  "Job epilog failed"			        	},
	{ ERRTAB_ENTRY(ESLURMD_SESSION_KILLED),
