...
extern int pack_ctld_job_step_info(void *x, void *arg)
{
	...
	if (args->proto_version >= SLURM_26_05_PROTOCOL_VERSION) {
		...
		packstr(step_ptr->runtime, buffer);
			   ...
			   uint16_t protocol_version)
{
	...
	if (protocol_version >= SLURM_26_11_PROTOCOL_VERSION) {
		...
		safe_unpack32_array(&cpu_alloc_reps, &cpu_alloc_array_cnt,
				    buffer);
		...
		if (gres_step_state_unpack(&gres_list_req, buffer, &step_id,
					   protocol_version) != SLURM_SUCCESS)
			goto unpack_error;
		if (gres_step_state_unpack(&gres_list_alloc, buffer, &step_id,
					   protocol_version) != SLURM_SUCCESS)
			goto unpack_error;
		...
		if (uint8_tmp && (switch_g_stepinfo_unpack(&switch_tmp, buffer,
							   protocol_version)))
			goto unpack_error;
				   ...
				   uint16_t protocol_version)
{
	if (protocol_version >= SLURM_26_05_PROTOCOL_VERSION) {
		...
		packstr(dump_job_ptr->runtime, buffer);
				    ...
				    uint16_t protocol_version)
{
	if (protocol_version >= SLURM_26_05_PROTOCOL_VERSION) {
		...
		safe_unpackstr(&job_ptr->runtime, buffer);
