...
static int cbuf_find_replay_line(dbuf_t *cb, int chars, int *nlines, int *nl);
static int cbuf_find_unread_line(dbuf_t *cb, int chars, int *nlines);
...
static int cbuf_copier(dbuf_t *src, dbuf_t *dst, int len, int *ndropped);
static int cbuf_dropper(dbuf_t *cb, int len);
static int cbuf_reader(dbuf_t *src, int len, cbuf_iof putf, void *dst);
static int cbuf_replayer(dbuf_t *src, int len, cbuf_iof putf, void *dst);
static int cbuf_writer(dbuf_t *dst, int len, cbuf_iof getf, void *src,
		       int *ndropped);

static int cbuf_grow(dbuf_t *cb, int n);
static int cbuf_shrink(dbuf_t *cb);

#ifndef NDEBUG
static int _cbuf_is_valid(dbuf_t *cb);
static int _cbuf_mutex_is_locked(dbuf_t *cb);
#endif /* !NDEBUG */
...
dbuf_t *cbuf_create(int minsize, int maxsize)
{
    dbuf_t *cb;
...
int cbuf_lines_reused(dbuf_t *cb)
{
    int lines = -1;
...
int cbuf_opt_get(dbuf_t *cb, cbuf_opt_t name, int *value)
{
    int rc = 0;
...
int cbuf_opt_set(dbuf_t *cb, cbuf_opt_t name, int value)
{
    int rc = 0;
...
int cbuf_drop(dbuf_t *src, int len)
{
    assert(src != NULL);
...
int cbuf_peek(dbuf_t *src, void *dstbuf, int len)
{
    int n;
...
int cbuf_read(dbuf_t *src, void *dstbuf, int len)
{
    int n;
...
int cbuf_replay(dbuf_t *src, void *dstbuf, int len)
{
    int n;
...
int cbuf_rewind(dbuf_t *src, int len)
{
    int reused;
...
int cbuf_write(dbuf_t *dst, void *srcbuf, int len, int *ndropped)
{
    int n;
...
int cbuf_drop_line(dbuf_t *src, int len, int lines)
{
    int n;
...
int cbuf_peek_line(dbuf_t *src, char *dstbuf, int len, int lines)
{
    int n, m, l;
    char *pdst;
...
int cbuf_read_line(dbuf_t *src, char *dstbuf, int len, int lines)
{
    int n, m, l;
    char *pdst;
...
int cbuf_replay_line(dbuf_t *src, char *dstbuf, int len, int lines)
{
    int n, m, l;
    int nl;
    char *pdst;
...
int cbuf_rewind_line(dbuf_t *src, int len, int lines)
{
    int n;
...
int cbuf_write_line(dbuf_t *dst, char *srcbuf, int *ndropped)
{
    int len;
    int nfree, ncopy, n;
    int ndrop = 0, d;
...
int cbuf_peek_to_fd(dbuf_t *src, int dstfd, int len)
{
    int n = 0;
...
int cbuf_read_to_fd(dbuf_t *src, int dstfd, int len)
{
    int n = 0;
...
int cbuf_replay_to_fd(dbuf_t *src, int dstfd, int len)
{
    int n = 0;
...
int cbuf_write_from_fd(dbuf_t *dst, int srcfd, int len, int *ndropped)
{
    int n = 0;
...
int cbuf_copy(dbuf_t *src, dbuf_t *dst, int len, int *ndropped)
{
    int n = 0;
...
int cbuf_move(dbuf_t *src, dbuf_t *dst, int len, int *ndropped)
{
    int n = 0;
...
static int cbuf_writer(dbuf_t *dst, int len, cbuf_iof getf, void *src, int *ndropped)
{
