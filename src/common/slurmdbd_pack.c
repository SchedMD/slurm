				 ...
				 buf_t *buffer)
{
	...
	*msg = msg_ptr;
	...
	if (rpc_version >= SLURM_22_05_PROTOCOL_VERSION) {
		...
		safe_unpackstr_xmalloc(&msg_ptr->work_dir, &uint32_tmp, buffer);
		if (env) {
			(void) hash_g_compute(env, strlen(env), NULL, 0, &hash);
			tmp = xstring_bytes2hex(hash.hash, sizeof(hash.hash),
						NULL);
			msg_ptr->env_hash = xstrdup_printf("%d:%s",
							   hash.type, tmp);
			xfree(env);
		}
		if (script) {
			(void) hash_g_compute(script, strlen(script), NULL, 0,
					      &hash);
			tmp = xstring_bytes2hex(hash.hash, sizeof(hash.hash),
						NULL);
			msg_ptr->script_hash = xstrdup_printf("%d:%s",
							      hash.type, tmp);
			xfree(script);
		}
	} else if (rpc_version >= SLURM_MIN_PROTOCOL_VERSION) {
