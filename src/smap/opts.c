...
extern void parse_command_line(int argc, char *argv[])
{
	...
	while((opt_char = getopt_long(argc, argv, "adD:ehi:j:ln:No:p:rRsS:t:vV",
	...
	if(params.Display==SLURMPART)
	  {
		...
		params.format = params.long_output ?  
	      ...
	      } else if ((env_val = getenv ("SMAP_FORMAT"))) {
		params.format = xstrdup(env_val);
...
/* 
 * parse_format_jobs - Take the user's format specification and use it to build 
