...
Hewlett-Packard Corporation, and Linux NetworX.
SLURM's general characteristics include:
Simplicity: SLURM is simple enough to allow motivated end users
  to understand its source code and add functionality. 
  It supports only a few simple scheduling algorithms, 
  but relies upon an external scheduler for sophisticated
  workload prioritization.
Open Source: SLURM is available to everyone and will remain free.
  Its source code is distributed under the GNU General Public
  License \cite{GPL2002}.
Portability: SLURM is written in the C language, with a GNU
...
Scalability: SLURM is designed for scalability to clusters of
  thousands of nodes. Time to fully execute (allocate, launch
...
Fault Tolerance SLURM can handle a variety of failure
  modes without terminating workloads.
Security: SLURM employs crypto technology to authenticate
  users to services and services to each other with a variety of options
  available through the plugin mechanism.  
System Administrator Friendly: SLURM utilizes
...
{\tt scontrol} is a system administrator tool to monitor and change
system configuration, 
{\tt sinfo} reports status and configuration of the nodes and queues,
{\tt squeue} reports status of jobs, 
{\tt scancel} can signal and/or cancel jobs or job steps
{\tt smap} reports node, queue, and job status including 
topological state information,
{\tt sacct} reports job accounting information, 
{\tt sbcast} copies a file to local file systems on a job's allocated nodes, and
{\tt srun} is used to allocate resources and spawn the job's tasks.
...
Very high scalability was treated as the most important design 
criterion for SLURM. 
...
processors.
While that is still a recommended mode of operation for very large 
clusters, an alternate SLURM plugin provides resource management 
down the the resolution of individual processors.
...
The combination of these tow features means that, 
for example, three users can concurrently get job state information
while the system administrator is modifying the time limit 
for a partition.
...
A number of interesting papers have recently been written about
...
In a simplified example, consider system daemons that run for 
one second out of every 100 seconds. For serial jobs this 
...
1. Making the {\tt slurmd} daemon resource requirements negligible 
2. Supporting configurations that let the {\tt slurmd} daemon sleep 
and not interfere at all with running jobs
3. If the {\tt slurmd} daemons are given work, it is done on a 
...
in terms of the data managed and this capability is provided by an 
optional plugin.
...
communicate in the task launch operation in order to minimize the 
...
Upon receipt of job termination confirmation, {\tt slurmctld} 
...
The cluster size is presently limited by the size of some 
data structures, but operation on larger systems is certainly 
feasible with minor code changes.

\section{Acknowledgments}

SLURM was jointly developed by LLNL, Hewlett-Packard Company
and Linux NetworX. A multitude of programmers have contributed 
to it's development.
