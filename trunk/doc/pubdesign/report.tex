...
controller for a cluster with 1000 nodes occupies on the order of 2 MB 
of memory and performance is excellent. 
...
its execution.  Once executing, the job manager monitors and records
...
{\tt authd}\footnote{http://www.cs.berkeley.edu/\~bnc/authd/} and 
...
Both the \slurmd\ and \slurmctld\ also support the use
...
A user wishes to run a job in batch mode, in which \srun\ returns 
...
blocks while the job executes and stdout/stderr of the job are 
...
A backup \slurmctld\, if one is configured, periodically pings
the primary {\tt slurmctld}.  Should the primary \slurmctld\ cease
responding, the backup loads state information from the last 
\slurmctld\ state save, and assumes control.  All \slurmd\ daemons
...
returned to service, it tells the backup \slurmctld\ to save
...
the /etc/slurm.conf file, captures system state, attempts an initial
...
\slurmd\ executes the prolog script (if any), resets its
...
\scancel\ terminate queued or running jobs or job steps.
...
A single machine serves as a centralized cluster manager and database. 
We do not anticipate user applications executing on this machine. 
...
IBM Blue Gene\footnote{http://www.llnl.gov/asci/platforms/bluegenel}
in 2003. 
