...
controller for a cluster with 16k nodes will occupy on the order of 2 MB 
...
of parallel jobs). However we intend to support the explicit preemption 
and later resumption of a job under the direction of an external scheduler.
...
status with \slurmctld\.  The only job information it has at any given time
...
over 1000 nodes without difficulty. We are reviewing other communication 
mechanisms which may offer improved scalability. One possible alternative 
is STORM \footnote{http:// whatever}. STORM uses the cluster interconnect 
and Network Interface Cards to provide a high-speed broadcast capability. 
STORM only supports the Quadrics Elan interconnnect at present, but does 
offer the promise of scalability to tens of thousands of nodes or beyond.
...
Access to some partitions is restricted via a `` RootOnly'' flag.  
...
prolog program (if one is configured), and executes the
...
scalability.  Full controller state information will be written to 
disk periodically with incremental changes written to disk immediately
for fault tolerance.  The controller  will include the
...
\begin{itemize}
\item Name
\item RootOnly flag to indicated that only user {\tt root} may initiate jobs 
...
would not be effected).  This information can be confined to the \slurmctld\
...
SlurmctldPort=1234 SlurmdPort=1235
Prolog=/usr/local/slurm/prolog Epilog=/usr/local/slurm/epilog
HeartbeatInterval=60 SlurmctldTimeout=300 SlurmdTimeout=300 KillWait=30
Prioritize=/usr/local/maui/priority
#
# Node Configurations
#
NodeName=DEFAULT TmpDisk=16384
NodeName=lx[0000-0002] State=DRAINED
...
The slurm daemon, {\tt slurmd}, is a multi-threaded daemon for managing
...
core paths, process limits (?), SLURM job id, and the command line to execute. 
...
through input specification of: SLURM job ID and/or job step ID. 
...
\begin{center}
  \begin{tabular}[t]{lcl}
   ...
   \hhline{---}
      ...
      {\em immediate}	   & boolean& exit if resources are not immediately
available  \\
      {\em overcommit}     & boolean& allow more than 1 process per cpu	      \\
      {\em label}	   & boolean& prepend task number to lines of stdout/err \\
      {\em distribution}   & string & distribution method for processes
(block$|$cyclic) \\
