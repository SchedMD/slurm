...
[
	...
	# test for bluegene emulation mode
   	AC_ARG_WITH(bluegene-emulation, AS_HELP_STRING(--enable-bluegene-emulation,Run SLURM in bluegene mode on a non-bluegene system), 
	[ case "$enableval" in
	  yes) bluegene-emulation=yes ;;
	  no)  bluegene-emulation=no ;;
