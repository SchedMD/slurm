/*****************************************************************************\
 *  printjob.c - smap print job functions
 *****************************************************************************
...
int _print_int(int number, int width, bool right, bool cut_output)
{
  ...
  retval = _print_str(buf, width, right, cut_output);
...
int _job_print_time(time_t t, int level, int width, bool right)
{
	struct tm time;
	char str[FORMAT_STRING_SIZE];
			  ...
			  char* suffix)
{
  if (job == NULL)	/* Print the Header instead */
    ...
    _print_secs((job->time_limit*60), width, right, true);
  if (suffix)
    {
      mvwprintw(text_win,ycord,xcord,"%s", suffix);
      xcord+=strlen(suffix);
			   ...
			   char* suffix)
{
  if (job == NULL)	/* Print the Header instead */
    ...
    _print_secs(job_time_used(job), width, right, true);
  if (suffix)
    {
      mvwprintw(text_win,ycord,xcord,"%s", suffix);
      xcord+=strlen(suffix);
			  ...
			  char* suffix)
{
  if (job == NULL)	/* Print the Header instead */
    ...
    {
      wattron(text_win,COLOR_PAIR(FillInValue[GridCount].color));
      _job_print_time(job->start_time, 0, width, right);
      wattroff(text_win,COLOR_PAIR(FillInValue[GridCount].color));
    }
  if (suffix)
    {
...
int _print_job_time_end(job_info_t * job, int width, bool right, char* suffix)
{
  if (job == NULL)	/* Print the Header instead */
    ...
    {
      wattron(text_win,COLOR_PAIR(FillInValue[GridCount].color));
      _job_print_time(job->end_time, 0, width, right);
      wattroff(text_win,COLOR_PAIR(FillInValue[GridCount].color));
    }
  if (suffix)
    {
...
{
  if (job == NULL)        /* Print the Header instead */
    _print_str("NODELIST", width, right, true);
  else
    {
      wattron(text_win,COLOR_PAIR(FillInValue[GridCount].color));
      _print_nodes(job->nodes, width, right, true);
    }
  if (suffix)
    {
      mvwprintw(text_win,ycord,xcord,"%s", suffix);
			   ...
			   char* suffix)
{
  if (step == NULL)	/* Print the Header instead */
    ...
    _job_print_time(step->start_time, 0, width, right);
  if (suffix)
    {
      mvwprintw(text_win,ycord,xcord,"%s", suffix);
      xcord+=strlen(suffix);
