		     ...
		     List format_list, List grouping_list)
{
	...
	if(!job_cond->usage_end) {
		...
		end_tm.tm_hour = 0;
		job_cond->usage_end = mktime(&end_tm);		
	} else {
		if(!localtime_r((time_t *)&job_cond->usage_end, &end_tm)) {
	...
	if(!job_cond->usage_start) {
		...
		if(!localtime_r((time_t *)&job_cond->usage_start,
		   &start_tm)) {
	...
	if(job_cond->usage_end-job_cond->usage_start < 3600) 
		job_cond->usage_end = job_cond->usage_start + 3600;
