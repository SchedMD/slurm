...
extern int slurm_spawn (slurm_step_ctx ctx, int *fd_array)
{
	...
	sock_array = xmalloc(ctx->num_tasks * sizeof(int));
	...
	msg_array_ptr = xmalloc(sizeof(spawn_task_request_msg_t) *
			ctx->num_tasks);
	req_array_ptr = xmalloc(sizeof(slurm_msg_t) * ctx->num_tasks);
	for (i=0; i<ctx->nhosts; i++) {
		for (j=0; j<ctx->tasks[i]; j++) {
			uint32_t tid = ctx->tids[i][j];
			spawn_task_request_msg_t *r = &msg_array_ptr[tid];
			slurm_msg_t                *m = &req_array_ptr[tid];
...
static int _p_launch(slurm_msg_t *req, slurm_step_ctx ctx)
{
	...
	thd = xmalloc(sizeof(thd_t) * ctx->num_tasks);
	if (thd == NULL) {
		slurm_seterrno(ENOMEM);
		return SLURM_ERROR;
	}
