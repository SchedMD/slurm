...
wait
    ...
    assert atf.wait_for_job_state(
        job_id, "DONE", timeout=60, poll_interval=2
    ), f"Job ({job_id}) did not run"
    assert (
        sacct_verify_unique_nodes(
            atf.run_command_output(
                f"sacct -j {job_id} --format=Nodelist --parsable2 --noheader"
            )
        )
        is True
    ), "Jobs ran on the same node"
