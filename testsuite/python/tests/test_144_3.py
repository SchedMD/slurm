...
def setup():
    ...
    atf.require_config_parameter('Name', {'gpu' : {'File' :
        gpu_file_pattern}}, source='gres')

    atf.make_bash_script('step.sh', '''
            echo 'NODE_ID:'$SLURM_NODEID 'STEP_ID:'$SLURM_STEP_ID
            sleep 1
            exit 0''')
    step_path = f"{atf.module_tmp_path}/step.sh"
    atf.make_bash_script('job.sh', f"""
            echo 'step 1'
...
def validate_job(job_id, out_file):
    ...
    with open(out_file, 'r') as f:
        ...
        assert 'NODE_ID:0 STEP_ID:0' in out, \
                'Expect the first node to be in first step'
        assert 'NODE_ID:1 STEP_ID:0' in out, \
                'Expect the second node to be in first step'
        assert 'NODE_ID:0 STEP_ID:1' in out, \
                'Expect the first node to be in second step'
        assert 'NODE_ID:1 STEP_ID:1' in out, \
...
def test_exact_gpu_full_resources():
    ...
    job_id = atf.submit_job_sbatch(f"--cpus-per-gpu=2 --gpus-per-node=2 -N2 \
