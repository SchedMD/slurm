...
def test_account():
    ...
    ), "Account information not processed in srun from env var"
    output = atf.run_command_output(f"scontrol show job {job_id}")
    assert (
        re.search(rf"Account={qa_acct.lower()}", output) is not None
    ), "Account information not processed from env var in scontrol"
