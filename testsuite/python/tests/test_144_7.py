...
def setup():
    ...
    atf.require_config_parameter('Name', {'gpu': {'File': '/dev/tty[0-7]'}},
                                 source='gres')
...
def test_gpus_per_node_parallel_1_delayed():
    ...
    atf.make_bash_script(job_file, f"""
    ...
    atf.make_bash_script(step_file, f"""
    ...
    job_id = atf.submit_job_sbatch(
            ...
            f"-o {job_output_file} -J test_job {job_file}", fatal=True)
    ...
    assert not re.search(r"STEP_ID:\d+ CUDA_VISIBLE_DEVICES:\d+,", output), \
            "Not all steps used only 1 GPU"
    # Verify a GPU was used 3 times
    assert len(re.findall(r"STEP_ID:\d+ CUDA_VISIBLE_DEVICES:\d+",
                          output)) == 3, "A GPU was not used 3 times"
    # Verify one step was delayed
    assert atf.check_steps_delayed(job_id, output, 1), \
            "One step should have been delayed"

    if constrain_devices:
        # Verify all GPUs are CUDA_VISIBLE_DEVICES:0 (with ConstrainDevices)
        assert len(re.findall(r"STEP_ID:\d+ CUDA_VISIBLE_DEVICES:0",
                              output)) == 3, \
        ...
        cuda_devices_used = re.findall(
                r"STEP_ID:\d+ CUDA_VISIBLE_DEVICES:(\d+)",
                output)
        assert len(set(cuda_devices_used)) > 1, \
...
def test_gpus_per_node_parallel(step_args):
    ...
    assert len(re.findall(r"STEP_ID:\d+ CUDA_VISIBLE_DEVICES:\d+",
                          output)) == 2, "A GPU was not used 2 times"
    # Verify all steps run in parallel
    assert not re.search(r"Step completed in JobId=\d+, retrying", output), \
    ...
    if constrain_devices:
        ...
        assert len(re.findall(r"STEP_ID:\d+ CUDA_VISIBLE_DEVICES:1",
                              output)) == 1, \
...
def test_gpus_per_node_different_gpus():
    ...
    step_2gpu = re.search(r"STEP_ID:\d+ CUDA_VISIBLE_DEVICES:(\d+),(\d+)",
                          output).groups()
    step_1gpu = re.search(r"STEP_ID:\d+ CUDA_VISIBLE_DEVICES:(\d+$)", output,
                          re.MULTILINE).groups()

    # Verify 1 step used 1 GPU and 2 steps used 2 GPUs
    assert len(step_1gpu) == 1 and len(step_2gpu) == 2, \
    ...
    if constrain_devices:
        ...
        # with 0 in a step
        assert step_2gpu[0] == "0" and step_1gpu[0] == "0", \
        ...
        # unique
        assert step_1gpu[0] not in step_2gpu, \
                "All CUDA_VISIBLE_DEVICES are not unique"
...
def test_gpus_per_node_with_gpus_per_task():
    ...
    assert not re.search(r"STEP_ID:\d+ CUDA_VISIBLE_DEVICES:\d+,\d+,",
                         output), "A step has more than 2 GPUs"
    # Verify all steps have 2 GPUs
    assert len(re.findall(r"STEP_ID:\d+ CUDA_VISIBLE_DEVICES:\d+,\d+",
                          output)) == 3, "Not all steps have 2 GPUs"
    # Verify two steps were delayed, one of them twice
    assert atf.check_steps_delayed(job_id, output, 2), \
...
def test_gpus_per_node_with_gpus():
    ...
    assert len(re.findall(r"STEP_ID:0 CUDA_VISIBLE_DEVICES:\d+",
                          output)) == 2, "Step 0 did not have access to 2 GPUs"
    # Verify step 1 had access to 2 GPUs
    assert len(re.findall(r"STEP_ID:1 CUDA_VISIBLE_DEVICES:\d+",
                          output)) == 2, "Step 1 did not have access to 2 GPUs"
    # Verify step 2 had access to 2 GPUs
    assert len(re.findall(r"STEP_ID:2 CUDA_VISIBLE_DEVICES:\d+",
                          output)) == 2, "Step 2 did not have access to 2 GPUs"
    # Verify one step was delayed
    assert atf.check_steps_delayed(job_id, output, 1), \
    ...
    if constrain_devices:
        ...
        cuda_val = []
        cuda_val.append(re.search(r"STEP_ID:0 CUDA_VISIBLE_DEVICES:(\d+)",
                                  output).group(1))
        cuda_val.append(re.search(r"STEP_ID:1 CUDA_VISIBLE_DEVICES:(\d+)",
                                  output).group(1))
        cuda_val.append(re.search(r"STEP_ID:2 CUDA_VISIBLE_DEVICES:(\d+)",
                                  output).group(1))
        # Verify two first steps use different GPUs (without ConstrainDevices)
        assert cuda_val[0] != cuda_val[1], \
        ...
        assert cuda_val[2] == cuda_val[0] or cuda_val[2] == cuda_val[1], \
...
def test_gpus_per_node_with_gpus_2_nodes():
    ...
    assert not re.search(r"STEP_ID:\d+ CUDA_VISIBLE_DEVICES:\d+,\d+,\d+,\d+,",
                         output), "Not all steps have less than 5 GPUs per node"
    # Verify step 0 used 2 nodes
    assert len(re.findall(r"STEP_ID:0 CUDA_VISIBLE_DEVICES:\d+",
                          output)) == 2, "Step 0 did not use 2 nodes"
    # Verify step 1 used 2 nodes
    assert len(re.findall(r"STEP_ID:1 CUDA_VISIBLE_DEVICES:\d+",
                          output)) == 2, "Step 1 did not use 2 nodes"
    # Verify step 2 used 2 nodes
    assert len(re.findall(r"STEP_ID:2 CUDA_VISIBLE_DEVICES:\d+",
                          output)) == 2, "Step 2 did not use 2 nodes"
    # Verify two steps were delayed, one of them twice
    assert atf.check_steps_delayed(job_id, output, 2), \
            "Two steps were not delayed or one of them was not delayed twice"
...
def test_gpus_per_node_with_gpus_per_task_3():
    ...
    assert not re.search(r"STEP_ID:\d+ CUDA_VISIBLE_DEVICES:\d+,\d+,\d+,\d+,",
                         output), "More than 4 GPUs are visible in a step"
    # Verify job has access to 4 GPUs
    assert len(re.findall(r"STEP_ID:0 CUDA_VISIBLE_DEVICES:\d+,\d+,\d+,\d+",
                          output)) == 4, "Job does not have access to 4 GPUs"
    # Verify step 1 has 3 tasks and 2 GPUs per task
    assert len(re.findall(r"STEP_ID:1 CUDA_VISIBLE_DEVICES:\d+,\d+",
                          output)) == 3, \
            "Step 1 does not have 3 tasks and 2 GPUs per task"
    # Verify step 2 has 3 tasks and 2 GPUs per task
    assert len(re.findall(r"STEP_ID:2 CUDA_VISIBLE_DEVICES:\d+,\d+",
                          output)) == 3, \
...
def test_gpus_per_node_with_gpus_per_task_5():
    ...
    assert not re.search(r"STEP_ID:\d+ CUDA_VISIBLE_DEVICES:\d+,\d+,\d+,",
                         output), "More that 2 GPUs are visible in a step"
    # Verify all 5 steps have access to 2 GPUs
    assert len(re.findall(r"STEP_ID:\d+ CUDA_VISIBLE_DEVICES:\d+,\d+",
                          output)) == 5, "Not all 5 steps have access to 2 GPUs"
    # Verify one step was delayed
    assert atf.check_steps_delayed(job_id, output, 1), \
            "One step was not delayed"
