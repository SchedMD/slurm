...
############################################################################
"""A job that reboots its nodes launches as soon as the last one registers.

SlurmctldParameters=fast_power_up_launch is documented to cover jobs waiting
on a reboot, not only jobs waiting on nodes to resume from power save.
reboot_job_nodes() marks the nodes NODE_STATE_POWERING_UP for a reboot
requested with sbatch --reboot, and that flag is what validate_node_specs()
gates the prompt launch on, so both triggers share one code path.
...
def setup():
    ...
    atf.require_nodes(2)
    # Rebooted nodes must come back into service on their own.
...
def test_job_launches_when_last_rebooted_node_registers():
    """A --reboot job is RUNNING by the time its last node leaves POWERING_UP."""
    ...
    # --reboot is restricted to root, SlurmUser and admins, so submit as
    # SlurmUser rather than the unprivileged test user.
    submitted = datetime.now()
    job_id = atf.submit_job_sbatch(
        f"-w {atf.node_list_to_range(nodes)} -N{len(nodes)} --reboot"
        " --wrap 'sleep infinity'",
        fatal=True,
        user=atf.properties["slurm-user"],
    )

    # reboot_job_nodes() sets POWERING_UP on every node it boots, which is the
    # flag validate_node_specs() gates the prompt launch on.
    for node in nodes:
        assert atf.wait_for_node_state(
            node, "POWERING_UP", timeout=60
        ), f"{node} should be POWERING_UP while the job reboots it"
