...
# Note that power_interval has to be at least 5 seconds, recommended 10 seconds for later tests
power_interval = 10
suspend_time = 10
suspend_timeout = 10
resume_timeout = 10
...
def setup():
    ...
    # Time for cloud node to sit idle with no jobs before being told to power down
    ...
    # Define nf1 feature in 'helpers.conf'. Used for testing Node Features in a later test
    atf.require_config_parameter("NodeName",
        {f"{node_prefix}1": {"Feature": "nf1", "Helper": "/bin/true"}}, source="helpers")

    # Don't run the usual atf.require_slurm_running() because tests will start slurmds manually
...
# Since our teardown doesn't seem to remove cloud nodes' slurmds, we need to delete the slurmds
def kill_slurmds():
...
# Since our teardown doesn't seem to remove slurmctlds started with "atf.start_slurmctld()", we
# need to delete the slurmctld
def kill_slurmctld():
...
# Tests
# Test state cycle of cloud nodes: POWERED_DOWN, POWERING_UP, IDLE, POWERING_DOWN, POWERED_DOWN
def test_cloud_state_cycle():
    ...
    # Schedule a job to cloud node's partition, transitioning node to ALLOCATED and POWERING_UP state
    ...
    # Make sure the cloud node starts suspending once it hasn't received a job for suspend_time
    atf.wait_for_node_state(f"{node_prefix}1", "POWERING_DOWN", timeout=suspend_time+5, fatal=True)
...
def test_resume_timeout():
    ...
    # Never spin up the accompanying slurmd, waiting resume_timeout for cloud node to be DOWN
    atf.wait_for_node_state(f"{node_prefix}1", "DOWN", timeout=resume_timeout+5, fatal=True)
    ...
    assert atf.cancel_all_jobs(), "There should be no more jobs remaining after canceling all jobs"
...
def test_scontrol_power_down_asap():
    ...
    # Wait for the job to finish and then assert cloud node reaches POWERED_DOWN state
...
# Test cloud nodes POWER_DOWN and then power up with different ActiveFeatures to handle jobs
def test_node_features():
    ...
    # Schedule a job to cloud node's partition, transitioning node to ALLOCATED and POWERING_UP state
    job_id = atf.submit_job_sbatch(f"-p cloud1 -C f1 --wrap 'srun hostname'", fatal=True)
    atf.wait_for_node_state(f"{node_prefix}1", "ALLOCATED", fatal=True)
    atf.wait_for_node_state(f"{node_prefix}1", "POWERING_UP", timeout=5, fatal=True)
    assert "CONFIGURING" == atf.get_job_parameter(job_id, "JobState", default="NOT_FOUND",
        quiet=True), \
    ...
    # Make sure cloud node maintains the expected configuration of features (not nf1)
    assert available_features == sorted(atf.get_node_parameter(
            f"{node_prefix}1", "AvailableFeatures").split(",")), \
        "Cloud node's AvailableFeatures don't match what was set in slurm.conf"
    ...
    # Now submit job requiring nf1 feature, which cloud node doesn't currently have active
    job_id = atf.submit_job_sbatch(f"-p cloud1 -C nf1 -w {node_prefix}1 --wrap 'srun hostname'", fatal=True)

    # Make sure cloud node enters POWERING_DOWN state without running the job
    atf.wait_for_node_state(f"{node_prefix}1", "POWERING_DOWN", timeout=suspend_time+5, fatal=True)
    assert "PENDING" == atf.get_job_parameter(job_id, "JobState", default="NOT_FOUND", quiet=True), \
    ...
    # Test that the cloud node now has the activated feature needed to run the job
    assert available_features == sorted(atf.get_node_parameter(
            f"{node_prefix}1", "AvailableFeatures").split(",")), \
        "Cloud node's AvailableFeatures don't match what was set in slurm.conf"
    assert available_features == sorted(atf.get_node_parameter(
...
def test_power_down_on_idle():
    ...
    # Schedule a job to cloud node's partition, transitioning node to ALLOCATED and POWERING_UP state
    job_id = atf.submit_job_sbatch(f"-p powerDownOnIdle --wrap 'srun hostname'", fatal=True)
    atf.wait_for_node_state(f"{node_prefix}1", "ALLOCATED", fatal=True)
    atf.wait_for_node_state(f"{node_prefix}1", "POWERING_UP", timeout=5, fatal=True)
    assert "CONFIGURING" == atf.get_job_parameter(job_id, "JobState", default="NOT_FOUND",
        quiet=True), \
    ...
    assert "POWER_DOWN" in atf.get_node_parameter(f"{node_prefix}1", "State").split("+") \
        ...
        "Cloud node wasn't immediately POWERING_DOWN once idle, in contrary to "\
        "'PowerDownOnIdle=Yes' flag for parition"
    atf.wait_for_node_state(f"{node_prefix}1", "POWERING_DOWN", timeout=5, fatal=True)
    atf.wait_for_node_state(f"{node_prefix}1", "POWERED_DOWN", timeout=suspend_timeout+5, fatal=True)
    assert "IDLE" in atf.get_node_parameter(f"{node_prefix}1", "State").split("+"), \
        "Per 'SlurmctldParameters=idle_on_node_suspend' in slurm.conf, cloud node should always be "\
...
# Make the check-in interval of the power save thread longer so we have time to POWER_DOWN_FORCE a
# cloud node that's already POWERED_DOWN and ALLOCATED before it's powered up. Saved as last test
# due to changing the slurm.conf file
def test_scontrol_power_down_force():
    ...
    # Submit job, get it assigned to the cloud node, and make sure everything goes well before cloud
    # node enters POWERING_UP state
    job_id = atf.submit_job_sbatch(f"-p cloud1 --wrap 'srun sleep 300'", fatal=True)
    atf.wait_for_node_state(f"{node_prefix}1", "ALLOCATED", timeout=5, poll_interval=0.1, fatal=True)
    assert "CONFIGURING" == atf.get_job_parameter(job_id, "JobState", default="NOT_FOUND",
        quiet=True), \
        "Submitted job should be CONFIGURING while there is a corresponding ALLOCATED cloud node"

    # Now POWER_DOWN_FORCE the cloud node, make sure it enters POWER_DOWN and never POWERING_UP,
    # and assure that the job gets requeued
    assert "POWERED_DOWN" in atf.get_node_parameter(f"{node_prefix}1", "State").split("+"), \
        "Couldn't run POWER_DOWN_FORCE on cloud node before it left POWERED_DOWN state. Try making "\
        "power_interval longer to give the test more time"
    atf.run_command(f"scontrol update nodename={node_prefix}1 state=POWER_DOWN_FORCE",
        fatal=True, user="slurm")
    assert "IDLE" in atf.get_node_parameter(f"{node_prefix}1", "State").split("+"), \
        "Per 'SlurmctldParameters=idle_on_node_suspend' in slurm.conf, cloud node should always be "\
    ...
    atf.wait_for_node_state(f"{node_prefix}1", "POWERING_DOWN", timeout=power_interval+5, fatal=True)
    atf.wait_for_node_state(f"{node_prefix}1", "POWERED_DOWN", timeout=power_interval+suspend_timeout+5, fatal=True)
