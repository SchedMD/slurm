...
static void	_part_op(bg_update_t *bg_update_ptr);
...
static void _bg_list_del(void *x)
{
	...
	if (bg_update_ptr) {
		xfree(bg_update_ptr->bg_part_id);
		xfree(bg_update_ptr);
	}
}
...
static void _sync_agent(bg_update_t *bg_update_ptr)
{
	...
	bg_record = find_bg_record(bg_update_ptr->bg_part_id);
	if(!bg_record) {
		error("No block %s", bg_update_ptr->bg_part_id);
		return;
	}
...
static void _term_agent(bg_update_t *bg_update_ptr)
{
	...
	for (i=0; i<jobs; i++) {		
		...
		if ((rc = rm_get_data(job_elem, RM_JobBlockID, &part_id))
		    != STATUS_OK) {
			error("rm_get_data(RM_JobPartitionID) %s: %s", 
			      part_id, bg_err_str(rc));
			continue;
		}
		...
		debug2("looking at block %s looking for %s\n",
			part_id, bg_update_ptr->bg_part_id);
			
		if (strcmp(part_id, bg_update_ptr->bg_part_id) != 0) {
			free(part_id);
			continue;
		}
	...
	if(bg_list) {
		...
		while ((block_ptr = (bg_record_t *) list_next(itr))) {
			if ((block_ptr->user_name == NULL)
			    ...
			    ||  (block_ptr->bg_part_id == NULL)
			    ||  (block_ptr->bg_part_id[0] == '0'))
				continue;
			str_ptr = xmalloc(sizeof(bg_record_t));
			str_ptr->bg_part_id = xstrdup(block_ptr->bg_part_id);
...
static int _excise_block(List block_list, pm_partition_id_t bg_part_id,
			 char *nodes)
{
	...
	if(block_list) {
		...
		while ((block = list_next(iter))) {
			rc = SLURM_ERROR;
			if (strcmp(block->bg_part_id, bg_part_id))
				continue;
			if (strcmp(block->nodes, nodes)) {	
				/* changed bgblock */
				error("bg_part_id:%s old_nodes:%s "
				      "new_nodes:%s",
				      bg_part_id, nodes, block->nodes);
				break;
			}
			
			/* exact match of name and node list */
			debug("synced Block %s", bg_part_id);
			list_delete(iter);
			rc = SLURM_SUCCESS;
			break;
		}		
 ...
 * can not wait.
 */
int term_jobs_on_part(pm_block_id_t bg_part_id)
{
	int rc = SLURM_SUCCESS;
	bg_update_t *bg_update_ptr;
	/* if (bg_update_list == NULL) { */
...
/* 	} */
	...
	bg_update_ptr->bg_part_id = xstrdup(bg_part_id);
	_part_op(bg_update_ptr);
...
extern int start_job(struct job_record *job_ptr)
{
	...
	select_g_get_jobinfo(job_ptr->select_jobinfo,
		SELECT_DATA_PART_ID, &(bg_update_ptr->bg_part_id));
	select_g_get_jobinfo(job_ptr->select_jobinfo,
	...
	if (bg_list) {
		...
		while ((found_record = (bg_record_t *) list_next(itr))) {
			if (bg_record->full_block)
				...
				found_record->state = RM_PARTITION_FREE;
			if ((!found_record->bg_part_id)
			    ||  (strcmp(part_id, found_record->bg_part_id)))
				continue;
...
int term_job(struct job_record *job_ptr)
{
	...
	info("Queue termination of job %u in BG block %s",
		job_ptr->job_id, bg_update_ptr->bg_part_id);
	_part_op(bg_update_ptr);
#else
...
extern int sync_jobs(List job_list)
{
	...
	if(job_list) {
		...
		while ((job_ptr = (struct job_record *) 
			...
			if (bg_update_ptr->bg_part_id == NULL) {
...
extern int boot_part(bg_record_t *bg_record)
{
	...
	if ((rc = rm_set_part_owner(bg_record->bg_part_id, 
				    slurmctld_conf.slurm_user_name)) 
	    != STATUS_OK) {
	...
	info("Booting block %s", 
	     bg_record->bg_part_id);
	if ((rc = pm_create_partition(bg_record->bg_part_id)) 
	    != STATUS_OK) {
		error("pm_create_partition(%s): %s",
		      bg_record->bg_part_id, bg_err_str(rc));
		return SLURM_ERROR;
	}
	rc = 0;
	while(rc < 10) {
	...
	debug("Setting bootflag for %s", bg_record->bg_part_id);
	bg_record->boot_state = 1;
	//bg_record->boot_count = 0;
	last_bg_update = time(NULL);
	slurm_mutex_unlock(&part_state_mutex);
