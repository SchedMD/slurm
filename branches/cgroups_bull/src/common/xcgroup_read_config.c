...
static void _clear_slurm_cgroup_conf(void)
{
	if (slurm_cgroup_conf) {
		...
		xfree(slurm_cgroup_conf->task_cgroup_params);
...
extern int read_slurm_cgroup_conf(void)
{
	s_p_options_t options[] = {
		...
		{"TaskAffinityBindType", S_P_STRING},
	...
	if ((conf_path == NULL) || (stat(conf_path, &buf) == -1)) {
		...
		/* user, job and jobsteps cgroup parameters */
		...
		/* Task Affinity parameters */
		slurm_cgroup_conf->task_bind_type = 0;
		if (s_p_get_string(&temp_str, "TaskAffinityBindType", tbl)) {
			char *last = NULL, *tok;
			...
			while (tok) {
				if (strcasecmp(tok, "none") == 0) {
					if (set_unit)
						fatal("Bad TaskAffinityBindType: %s", tok);
					set_unit = true;
					...
					if (set_unit)
						fatal("Bad TaskAffinityBindType: %s", tok);
					set_unit = true;
					slurm_cgroup_conf->task_bind_type |= CPU_BIND_TO_SOCKETS;
				} else if (strcasecmp(tok, "cores") == 0) {
					if (set_unit)
						fatal("Bad TaskAffinityBindType: %s", tok);
					set_unit = true;
					slurm_cgroup_conf->task_bind_type |= CPU_BIND_TO_CORES;
				} else if (strcasecmp(tok, "threads") == 0) {
					if (set_unit)
						fatal("Bad TaskAffinityBindType: %s", tok);
					set_unit = true;
					slurm_cgroup_conf->task_bind_type |= CPU_BIND_TO_THREADS;
				} else if (strcasecmp(tok, "sched") == 0) {
					if (set_mode)
						fatal("Bad TaskAffinityBindType: %s", tok);
					set_mode = true;
					/* No change to task_bind_type,
					 * this is the default */
				} else if (strcasecmp(tok, "verbose") == 0) {
					slurm_cgroup_conf->task_bind_type |= CPU_BIND_VERBOSE;
				} else
					fatal("Bad TaskAffinityBindType: %s", tok);
