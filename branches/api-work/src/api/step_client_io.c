...
static void     _init_stdio_eio_objs(client_io_t *cio);
 ...
 **********************************************************************/
extern eio_obj_t *
create_file_write_eio_obj(int fd, client_io_t *cio)
{
 ...
 **********************************************************************/
extern eio_obj_t *
create_file_read_eio_obj(int fd, client_io_t *cio,
...
static bool _file_readable(eio_obj_t *obj)
{
	...
	if (info->cio->ioservers_ready < info->cio->nhosts) {
...
again:
	...
	if (header.type == SLURM_IO_ALLSTDIN) {
		...
		msg->ref_count = 1;
		nodeid = step_layout_host_id(info->cio->step_layout, 
				             header.gtaskid);
 ...
 */
extern int 
client_io_node_fail(char *nodelist, client_io_t *cio)
{
...
static void
_init_stdio_eio_objs(client_io_t *cio)
{
	 ...
	 */
	if (_is_local_file(cio->ifname)) {
		uint16_t type, destid;
		if (cio->ifname->name == NULL || cio->ifname->taskid != -1) {
			infd = STDIN_FILENO;
		} else {
			infd = open(cio->ifname->name, O_RDONLY);
			if (infd == -1)
				fatal("Could not open stdin file: %m");
		}
		fd_set_nonblocking(infd);
		fd_set_close_on_exec(infd);
		if (cio->ifname->type == IO_ONE) {
			type = SLURM_IO_STDIN;
			destid = cio->ifname->taskid;
		} else {
			type = SLURM_IO_ALLSTDIN;
			destid = -1;
		}
		cio->stdin_obj = create_file_read_eio_obj(infd, cio,
							  type, destid);
	 ...
	 */
	if (_is_local_file(cio->ofname)) {
		int refcount;
		if (cio->ofname->name == NULL) {
			outfd = STDOUT_FILENO;
		} else {
			outfd = open(cio->ofname->name,
				     O_CREAT|O_WRONLY|O_TRUNC, 0644);
			if (outfd == -1)
				fatal("Could not open stdout file: %m");
		}
		if (cio->ofname->name != NULL
		    && cio->efname->name != NULL
		    && !strcmp(cio->ofname->name, cio->efname->name)) {
			refcount = cio->ntasks * 2;
			err_shares_out = true;
		} else {
			refcount = cio->ntasks;
		}
		cio->stdout_obj = create_file_write_eio_obj(outfd, cio);
	...
	} else if (_is_local_file(cio->efname)) {
		int refcount;
		if (cio->efname->name == NULL) {
			errfd = STDERR_FILENO;
		} else {
			errfd = open(cio->efname->name,
				     O_CREAT|O_WRONLY|O_TRUNC, 0644);
			if (errfd == -1)
				fatal("Could not open stderr file: %m");
		}
		refcount = cio->ntasks;
		cio->stderr_obj = create_file_write_eio_obj(errfd, cio);
...
client_io_t *
client_io_handler_create(int infd, int outfd, int errfd, bool labelio)
{
	client_io_t *c;

	c = (client_io_t *)xmalloc(sizeof(client_io_t));
	if (c == NULL)
		return NULL;
