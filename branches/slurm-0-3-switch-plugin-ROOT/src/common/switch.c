...
typedef struct slurm_switch_ops {
	...
	int          (*free_jobinfo)      ( switch_jobinfo_t jobinfo );
	int          (*pack_jobinfo)      ( switch_jobinfo_t jobinfo, 
						Buf buffer );
	int          (*unpack_jobinfo)    ( switch_jobinfo_t *jobinfo, 
						Buf buffer );
	int          (*prog_init)         ( switch_jobinfo_t jobinfo,
						uid_t uid);
	int          (*setcap)            ( switch_jobinfo_t switch_job,
						int procnum);
	int          (*prog_signal)       ( switch_jobinfo_t jobinfo,
						int signal);
	int          (*prog_destroy)      ( switch_jobinfo_t jobinfo );
	void         (*prog_fini)         ( switch_jobinfo_t jobinfo );
...
_slurm_switch_get_ops( slurm_switch_context_t c )
{
	...
	static const char *syms[] = {
		"slurm_libstate_save",
		"slurm_libstate_restore",
		"slurm_alloc_jobinfo",
		"slurm_build_jobinfo",
		"slurm_copy_jobinfo",
		"slurm_free_jobinfo",
		"slurm_pack_jobinfo",
		"slurm_unpack_jobinfo",
		"slurm_prog_init",
		"slurm_setcap",
		"slurm_prog_signal",
		"slurm_prog_destroy",
...
extern int g_switch_free_jobinfo(switch_jobinfo_t jobinfo)
{
	...
	return (*(g_context->ops.free_jobinfo))( jobinfo );
}
...
extern int  g_switch_prog_init (switch_jobinfo_t jobinfo, uid_t uid)
{
	if ( g_switch_init() < 0 )
	 ...
	 return (*(g_context->ops.prog_init)) (jobinfo, uid);
}
...
extern int  g_switch_setcap (switch_jobinfo_t jobinfo, int procnum)
{
	...
	return (*(g_context->ops.setcap)) ( jobinfo, procnum );
}
...
extern void  g_switch_prog_fini (switch_jobinfo_t jobinfo )
{
	...
	(*(g_context->ops.prog_fini)) ( jobinfo );
}
...
extern int  g_switch_prog_signal(switch_jobinfo_t jobinfo, int signal)
{
	if ( g_switch_init() < 0 )
        ...
        return (*(g_context->ops.prog_signal)) (jobinfo, signal);
}
...
extern int  g_switch_prog_destroy(switch_jobinfo_t jobinfo)
{
	...
	return (*(g_context->ops.prog_destroy)) (jobinfo);
}
