...
extern int ll_close(LL_element *jobmgmtObj) 
{
	...
	slurm_job_init_t *slurm_job_elem_ptr;
	resource_allocation_response_msg_t * job_alloc_resp;

	VERBOSE(stderr, "++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	VERBOSE(stderr, "ll_close\n");
	...
	slurm_job_elem_ptr = (slurm_job_init_t *) slurm_elem->data;
	job_alloc_resp = slurm_job_elem_ptr->job_alloc_resp;
	if (job_alloc_resp == NULL) {
		VERBOSE(stderr, "no resource allocation was made\n");
		goto complete;
	}

	VERBOSE (stderr, "cancelling slurm job %u\n", job_alloc_resp->job_id);
	if (slurm_complete_job(job_alloc_resp->job_id, 0, 0) < 0) {
		ERROR(stderr, "ERROR: slurm_complete_job: %s\n", 
