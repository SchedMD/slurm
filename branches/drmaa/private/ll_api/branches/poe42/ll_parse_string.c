 ...
 *                 -5: System error.
 *
 *  NOTE: SLURM can not accommodate an arbitrary task layout on the nodes,
 *  but only block or cyclic layout. To insure that the user does not specify 
 *  a hostlist for arbitrary task layout, we require the specification of 
 *  "requirements = (Feature == <partition_name>)" and do not use the 
 *  default SLURM partition unless specified.
 *****************************************************************************
		...
		LL_element **error_object)
{
	...
	VERBOSE(stderr, "++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	VERBOSE(stderr, "ll_parse_string\n");
	...
	if (jobstring == NULL) {
		ERROR(stderr, "ERROR: jobstring == NULL\n");
		rc = -5;
		goto done;
	}
	if (job == NULL) {
	...
	if (error_object == NULL) {
		ERROR(stderr, "ERROR: error_object == NULL\n");
		rc = -5;
		goto done;
	}
	...
	do {
		...
		if (key == INVALID_KEY) {
			ERROR(stderr, 
				"ERROR: invalid jobstring key specified %s\n", 
				line_beg);
			rc = -5;
			goto done;
		}
		...
		if (_process_key(slurm_elem, key, val) < 0) {
			ERROR(stderr, "ERROR: invalid llpp_parms value "
				"specified %s\n", line_beg);
			rc = -5;
			goto done;
		}
...
static int _validate_job(slurm_elem_t *elem)
{
	...
	if (slurm_job_ptr->partition == NULL) {
		/* Without pool specification a hostlist can be specified
		 * with an arbitrary task layout, which SLURM can not
		 * support at this time */
		ERROR(stderr, "ERROR: No pool name specified\n");
		return -1;
	}

	if ((slurm_job_ptr->min_nodes < 1)  &&
	    (slurm_job_ptr->num_tasks < 1)) {
		ERROR(stderr, "ERROR: Invalid node/task count specified: "
...
static int _parse_string(char *ll_str, enum cmd_keys *key, char **val)
{
	...
	if ((ll_str == NULL) || (key == NULL) || (val == NULL)) {
		ERROR(stderr, "ERROR: _parse_string argument NULL\n");
		return -1;
	}

	if ((ll_str[0] != '#') || (ll_str[1] != '@')) {
		ERROR(stderr, "ERROR: bad argument: %s\n", ll_str);
		return -1;
	}
...
enum cmd_keys _get_key_enum(char *key_str)
{
	...
	ERROR(stderr, "ERROR: currently unsupported key: %s\n", key_str);
	return INVALID_KEY;
}
...
static int _process_key(slurm_elem_t *elem, enum cmd_keys key, char *val)
{
	...
	case QUEUE_KEY:
		if (steps_queued++ > 0) {
			ERROR(stderr, "ERROR: Multiple steps queued\n");
			return -1;
		}
		return 0;
	case ERROR_KEY:
	...
	case MAX_PROCESSORS_KEY:
		nodes = atoi(val);
		if (nodes <= 0) {
			ERROR(stderr, "ERROR: invalid node count %d\n",
			      nodes);
			return -1;
		}
	...
	case TOTAL_TASKS_KEY:
		tasks = atoi(val);
		if (tasks <= 0) {
			ERROR(stderr, "ERROR: invalid task count %d\n",
			      tasks);
			return -1;
		}
	...
	case WALL_CLOCK_LIMIT_KEY:
		ERROR(stderr, "ERROR: Need parsing function here\n");
		break;
	case BULKXFER_KEY:
		VERBOSE(stderr, "hey we need to set "
			"the BULKXFER to something\n");
		break;
	...
	case STEP_NAME_KEY:	/* SLURM names jobs, not steps */
		...
		break;
	default:
		ERROR(stderr, "ERROR: unsupported job option '%s'\n", 
		      _key_to_str(key));
		return 0;
	}

	VERBOSE(stderr, "set '%s' to '%s'\n", _key_to_str(key), val);
	return 0;
}
...
static void _parse_requirements(job_desc_msg_t *slurm_job_ptr, char *val)
{
	...
	VERBOSE(stderr, "setting partition to '%s'\n", tmp);
