...
extern enum StepState remap_slurm_state(enum job_states slurm_job_state)
{
	...
	switch (slurm_job_state) {
		...
		default:
			ERROR(stderr, "remap_slurm_state(%d) unsupported\n",
					slurm_job_state);
			return STATE_COMPLETED;
	}
}
...
/* Logging functions */
#if DEBUG <= 2
extern void no_log(FILE *stream, const char *format, ...)
{
	/* don't log anything */
}
#endif
...
extern int build_step_ctx(slurm_elem_t *job_elem, slurm_elem_t *step_elem)
{
	...
	if (step_data->ctx == NULL) {
		ERROR(stderr, "ERROR: slurm_step_ctx_create: %s\n", 
		...
		slurm_job_init_t *slurm_job_init_ptr)
{
	...
	if (slurm_job_init_ptr->slurm_job_desc == NULL) {
		ERROR(stderr, "ERROR: slurm_job_desc is NULL");
		return NULL;
	}
	job_req = slurm_job_init_ptr->slurm_job_desc;
	if (slurm_job_init_ptr->job_alloc_resp == NULL) {
		ERROR(stderr, "ERROR: job_alloc_resp is NULL");
		return NULL;
	}
	...
	if (r == NULL) {
		ERROR(stderr, "ERROR: calloc error");
		return NULL;
	}
