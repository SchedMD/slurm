			     ...
			     void *resulting_data_type)
{
	...
	case LL_AdapterUsageAddress:
		...
		/* appears it can be null (0.0.0.0) */
		*resulting_char_data = "0.0.0.0"; 
				       ...
				       char **node_name)
{
	...
	*node_name = hostset_shift(step_data->host_set_copy);
	...
	node_data->node_addr = strdup(pHostInfo->h_addr);
			       ...
			       void *resulting_data_type)
{
	...
	case LL_TaskInstanceMachineAddress:
		...
		tmp_elem_ptr = node_data->step_elem;
		step_data = tmp_elem_ptr->data;
		tmp_elem_ptr = step_data->job_init_elem;
		job_data = tmp_elem_ptr->data;
		slurm_job_desc = job_data->slurm_job_desc;
		_parse_network_string(slurm_job_desc->network,
				      &job_data->protocol,
				      &job_data->mode,
				      &job_data->device);	
		if(!strcmp(job_data->mode,"ip"))
			*resulting_char_data = node_data->node_addr;
		else
			*resulting_char_data = NULL;
		if(*resulting_char_data)
			VERBOSE(stderr, "LL_TaskInstanceMachineAddress = %s\n",
				*resulting_char_data);	
		else
			VERBOSE(stderr, "LL_TaskInstanceMachineAddress "
				"= NULL\n");				
...
static slurm_elem_t *_build_adapter(slurm_elem_t *taski_elem)
{
	...
	adapter_data->taski_elem = taski_elem;
	
	adapter_data->protocol = job_data->protocol;
	adapter_data->mode = job_data->mode;
	adapter_data->device = job_data->device;
	
	if(!strcmp(adapter_data->device,"sn_all")) {
		free(adapter_data->device);
		adapter_data->device = "sn0";
		device = "sni0";
	...
	if(error_code != NTBL_SUCCESS) 
		...
		adapter_data->network_id = res.lid;
