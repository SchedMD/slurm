		...
		int *number_of_objs, int *error_code)
{
	...
	VERBOSE(stderr, "++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	if (error_code == NULL) {
		ERROR(stderr, "ERROR: ll_get_objs: error_code==NULL\n");
		goto done;
	}
	if (number_of_objs == NULL) {
		ERROR(stderr, "ERROR: ll_get_objs: number_of_objs==NULL\n");
		*error_code = -7;
		goto done;
	}
	if (query_element == NULL) {
		ERROR(stderr, "ERROR: ll_get_objs: query_element==NULL\n");
		*error_code = -1;
		goto done;
	}
	VERBOSE(stderr, "ll_get_objs: type=%s\n",
	...
	switch (slurm_elem->type) {
		case CLUSTER_QUERY:
			...
			break;
		default:
			ERROR(stderr, "ERROR: ll_get_objs: type=%s unsupported\n",
				query_type_str(slurm_elem->type));
			*error_code = -1;
	}
...
static LL_element * _build_cluster_elem(slurm_elem_t *cluster_query)
{
	...
	if (cluster_query->data == NULL) {
		ERROR(stderr, "ERROR: cluster_query->data == NULL\n");
