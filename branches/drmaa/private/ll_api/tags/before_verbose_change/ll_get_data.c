		       ...
		       void *resulting_data_type)
{
	...
	VERBOSE(stderr, "++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	VERBOSE(stderr, "ll_get_data\n");

	if (element == NULL) {
		ERROR(stderr, "ERROR: element is NULL, spec=%u\n", 
		      specification);
		rc = -1;
		goto done;
	}
	if (resulting_data_type == NULL) {
		ERROR(stderr, "ERROR: resulting_data_type is NULL, spec=%u\n",
		      specification);
		rc = -1;
		goto done;
	}
	VERBOSE(stderr, "element=%s, spec=%d\n",
	...
	case TASK_INST_ELEM:
		...
		break;
	default:
		ERROR(stderr, "ERROR: ll_get_data: %s element type "
		      "unsupported\n", 
			     ...
			     void *resulting_data_type)
{
	...
	case LL_AdapterUsageProtocol:
		...
		*resulting_char_data =
		...
		VERBOSE(stderr, "LL_AdapterUsageProtocol = %s\n",
			*resulting_char_data);
		break;
	case LL_AdapterUsageMode:
		...
		*resulting_char_data =
		...
		VERBOSE(stderr, "LL_AdapterUsageMode = %s\n",
			*resulting_char_data);
		break;
	case LL_AdapterUsageAddress:
		...
		*resulting_char_data = slurm_adapter_elem_ptr->address;
		...
		VERBOSE(stderr, "LL_AdapterUsageAddress = %s\n",
			*resulting_char_data);
		break;
	case LL_AdapterUsageWindow:
		...
		*resulting_int_data = 
		...
		VERBOSE(stderr, "LL_AdapterUsageWindow = %d\n", 
			*resulting_int_data);
		break;
	case LL_AdapterUsageDevice:
		...
		*resulting_char_data = 
		...
		VERBOSE(stderr, "LL_AdapterUsageDevice = %s\n",
			*resulting_char_data);
		break;
	case LL_AdapterUsageInstanceNumber:
		/* int * :Unique ID for multiple instances  */
		/* FIX ME! Hardcoded to zero for now */
		*resulting_int_data = 0;
		VERBOSE(stderr, "LL_AdapterUsageInstanceNumber = %d\n",
			*resulting_int_data);
		break;
	case LL_AdapterUsageNetworkId:
		/* int *: Network ID of adapter being used */
		/* FIX ME! Hardcoded to 2 for now this is the 
		   interface number from ifconfig */
		*resulting_int_data = slurm_adapter_elem_ptr->network_id;
		VERBOSE(stderr, "LL_AdapterUsageNetworkId = %d\n",
			*resulting_int_data);
		break;
	default:
		ERROR(stderr, "ERROR: ll_get_data: unsupported "
		      "spec=%d\n", specification);
		rc = -2;
	}
...
/*	slurm_cluster_elem_t *slurm_cluster_elem_ptr = (slurm_cluster_elem_t *)
	...
	case LL_ClusterSchedulerType:
		...
		*resulting_char_data = "slurm";
		VERBOSE(stderr, "LL_ClusterSchedulerType = %s\n",
			*resulting_char_data);
		break;
	default:
		ERROR(stderr, "ERROR: ll_get_data: unsupported "
			 ...
			 void *resulting_data_type)
{
	...
	case LL_JobGetFirstStep:
		...
		VERBOSE(stderr, "LL_JobGetFirstStep = step[0]\n");
		...
		*resulting_elem_data = job_data->first_step_elem;
		if (job_data->job_state >= JOB_RUNNING) {
			(void) build_step_ctx(slurm_elem, 
					      job_data->first_step_elem);
		}
		break;
			  ...
			  void *resulting_data_type)
{
	...
	case LL_NodeTaskCount:
		...
		*resulting_int_data = node_data->task_cnt;
		VERBOSE(stderr, "LL_NodeTaskCount = %d\n", 
			*resulting_int_data);
		break;
	case LL_NodeGetFirstTask:
		...
		*resulting_elem_data = _get_node_first_task(slurm_elem);
		if (*resulting_elem_data)
			VERBOSE(stderr, "LL_NodeGetFirstTask = task[0]\n");
		else
			ERROR(stderr, "LL_NodeGetFirstTask = NULL\n");
		break;
	case LL_NodeGetNextTask:
		/* LL_element * (Task) : next task	*/
		*resulting_elem_data = _get_node_next_task(slurm_elem);
		if (*resulting_elem_data)
			VERBOSE(stderr, "LL_NodeGetNextTask\n");
		else
			ERROR(stderr, "LL_NodeGetNextTask = NULL\n");
		break;
	default:
		ERROR(stderr, "ERROR: ll_get_data: unsupported "
		      "spec=%d\n", specification);
...
static LL_element *_get_node_next_task(slurm_elem_t *slurm_elem)
{
	...
	if ((node_data->next_task_inx < 0) ||
		...
		ERROR(stderr, "ERROR: Invalid task count on node %s\n",
		      node_data->node_name);
		return NULL;
	}
			  ...
			  void *resulting_data_type)
{
	...
	case LL_StepLargePage:
		...
		*resulting_char_data = strdup("M");
		VERBOSE(stderr, "LL_LargePage = %s\n", 
			*resulting_char_data);
		break;
	case LL_StepBulkXfer:
		/* int * : 0|1 Step Requests Bulk Transfer */
		/* FIXME: figure out what needs to happen here */
		*resulting_int_data = 0;
		VERBOSE(stderr, "LL_StepBulkXfer = %d\n", 
			*resulting_int_data);
		break;
	case LL_StepTotalRcxtBlocks:
		/* int * : total number of RCXT blocks 
		   application needs */
		*resulting_int_data = 0;
		VERBOSE(stderr, "LL_StepTotalRcxtBlocks = %d\n", 
			*resulting_int_data);
		break;
	case LL_StepMessages:
		...
		if (job_data->messages)
			...
			*resulting_char_data = strdup("");
		VERBOSE(stderr, "LL_StepMessages = %s\n",
			*resulting_char_data);
		break;
	...
	case LL_StepJobClass:
		...
		/* Not applicable to SLURM		*/
		*resulting_char_data = strdup("");
		VERBOSE(stderr, "LL_StepJobClass = %s\n",
			*resulting_char_data);
		break;
	case LL_StepMaxProtocolInstances:
		...
		/*         on network stmt */
		*resulting_int_data = 0;
		VERBOSE(stderr, "LL_StepMaxProtocolInstances = %d\n",
			*resulting_int_data);
		break;
	case LL_StepGetFirstSwitchTable:
		...
		*resulting_elem_data = _build_switch(slurm_elem);
		if (*resulting_elem_data)
			VERBOSE(stderr, "LL_StepGetFirstSwitchTable\n");
		else
			VERBOSE(stderr, 
				"LL_StepGetFirstSwitchTable = NULL\n");
		break;
	case LL_StepNodeCount:
		/* int * : number of nodes running the step	*/
		*resulting_int_data = _get_step_node_cnt(step_data);
		VERBOSE(stderr, "LL_StepNodeCount = %d\n", 
			*resulting_int_data);
		break;
	case LL_StepGetFirstNode:
		...
		*resulting_elem_data = _get_step_first_node(
		...
		if (*resulting_elem_data)
			VERBOSE(stderr, "LL_StepGetFirstNode = %s\n",
				node_name);
		else
			ERROR(stderr, "LL_StepGetFirstNode = NULL\n");
		break;
	case LL_StepGetNextNode:
		...
		*resulting_elem_data = _get_step_next_node(
			slurm_elem, &node_name);
		if (*resulting_elem_data)
			VERBOSE(stderr, "LL_StepGetNextNode = %s\n",
				node_name);
		else
			ERROR(stderr, "LL_StepGetNextNode = NULL\n");
		break;
	case LL_StepGetFirstMachine:
		...
		*resulting_elem_data = _build_machine(slurm_elem);
		if (*resulting_elem_data)
			VERBOSE(stderr, 
				"LL_StepGetFirstMachine = %s\n",
				node_name);
		else
			ERROR(stderr, 
			      "LL_StepGetFirstMachine = NULL\n");
		break;
	case LL_StepGetNextMachine:
		...
		*resulting_elem_data = _build_machine(slurm_elem);
		if (*resulting_elem_data)
			VERBOSE(stderr, 
				"LL_StepGetNextMachine = %s\n",
				node_name);
		else
			ERROR(stderr, 
			      "LL_StepGetNextMachine = NULL\n");
		break;
	default:
		ERROR(stderr, "ERROR: ll_get_data: unsupported "
		      "spec=%d\n", specification);
...
static slurm_elem_t *_build_switch(slurm_elem_t *step_elem)
{
	...
	if(slurm_step_ctx_get(step_data->ctx, 
			      SLURM_STEP_CTX_RESP, 
			      &resp_msg) < 0) {
		ERROR(stderr, "ERROR: step_ctx_get RESP: %s\n",
		      slurm_strerror(slurm_get_errno()));
		return rc;
	}
	jobinfo = resp_msg->switch_job;
	if(!jobinfo)
		ERROR(stderr, "Hey this jobinfo isn't set\n");
	...
	if(slurm_jobinfo_ctx_get(
		...
		ERROR(stderr, "ERROR: jobinfo_ctx_get RESP: %s\n",
		      slurm_strerror(slurm_get_errno()));
		return rc;
	}	
...
static slurm_elem_t *_build_machine(slurm_elem_t *step_elem)
{
	...
	if(slurm_jobinfo_ctx_get(
		   jobinfo, FED_JOBINFO_KEY, &key) < 0) {
		ERROR(stderr, "ERROR: machine jobinfo_ctx_get RESP: %s\n",
		      slurm_strerror(slurm_get_errno()));
		return rc;
	}	
					...
					char **node_name)
{
	...
	if ((step_data->job_init_elem == NULL) ||
	    ((job_data = (slurm_job_init_t *) step_data->job_init_elem->data) 
	     == NULL)) {
		ERROR(stderr, "ERROR: slurm_step_elem lacks job_init_elem\n");
		*node_name = NULL;
	...
	if (step_data->host_set == NULL) {
		ERROR(stderr, "ERROR: hostset_create failure for %s\n",
		      job_data->job_alloc_resp->node_list);
		*node_name = NULL;
		return (LL_element *) NULL;
	}
				       ...
				       char **node_name)
{
	...
	if (step_data->host_set_copy == NULL) {
		ERROR(stderr, "ERROR: called LL_GetNextNode before "
		      "LL_GetFirstNode\n");
		*node_name = NULL;
		return (LL_element *) NULL;
	}
	...
	*node_name = hostset_shift(step_data->host_set_copy);
	if (*node_name == NULL) {
		VERBOSE(stderr, "no more hosts in list\n");
		return (LL_element *) NULL;
	}
	...
	if((pHostInfo=gethostbyname(node_data->node_name)) == NULL) {
		printf("ERROR: base::init getHostbyname "
		       "returned NULL for %s\n", node_data->node_name);
		free(node_data);
		return (LL_element *) NULL;
	}
...
static int _get_task_cnt(slurm_step_elem_t *step_data, int node_inx)
{
	if (step_data->tasks_per_node == NULL) {
		if (slurm_step_ctx_get(step_data->ctx, 
			...
			ERROR(stderr, "ERROR: step_ctx_get TASKS: %s\n",
			      slurm_strerror(slurm_get_errno()));
			return 0;
		}
	}
	...
	if (step_data->tasks_per_node)
		...
		ERROR(stderr, "ERROR: tasks_per_node is NULL\n");
		return 0;
	}
}
			  ...
			  slurm_node_elem_t *node_data, int node_inx)
{
	if (slurm_step_ctx_get(step_data->ctx,
		...
		ERROR(stderr, "ERROR: step_ctx_get TID: %s\n",
		      slurm_strerror(slurm_get_errno()));
	}
}
			    ...
			    void *resulting_data_type)
{
	...
	case LL_SwitchTableJobKey:
		...
		*resulting_int_data = switch_data->job_key;
		VERBOSE(stderr, "LL_SwitchTableJobKey = %d\n",
			*resulting_int_data);
		break;
	default:
		ERROR(stderr, "ERROR: ll_get_data: unsupported "
...
/*	slurm_task_elem_t *task_data = (slurm_task_elem_t *)
	...
	case LL_TaskTaskInstanceCount:
		...
		*resulting_int_data = 1;
		VERBOSE(stderr, "LL_TaskTaskInstanceCount = %d\n",
			*resulting_int_data);
		break;
	case LL_TaskGetFirstTaskInstance:
		...
		*resulting_elem_data = _build_taski(slurm_elem);
		if (*resulting_elem_data)
			VERBOSE(stderr, "LL_TaskGetFirstTaskInstance\n");
		else
			VERBOSE(stderr, 
				"LL_TaskGetFirstTaskInstance = NULL\n");
		break;
	case LL_TaskGetNextTaskInstance:
		...
		*resulting_elem_data = NULL;
		if (*resulting_elem_data)
			VERBOSE(stderr, "LL_TaskGetNextTaskInstance\n");
		else
			VERBOSE(stderr, 
				"LL_TaskGetNextTaskInstance = NULL\n");
		break;
	default:
		ERROR(stderr, "ERROR: ll_get_data: unsupported "
		      "spec=%d\n", specification);
			       ...
			       void *resulting_data_type)
{
	...
	case LL_TaskInstanceAdapterCount:
		...
		/* FIX ME!  Federation switch plugin only supports
		 * 1 adapter/win pair per task for now, so this 
		 * will always be 1. If higher doesn't seem to work.
		 */
		*resulting_int_data = 1;
		VERBOSE(stderr, "LL_TaskInstanceAdapterCount = %d\n",
			*resulting_int_data);
		break;
	case LL_TaskInstanceGetFirstAdapter:
		...
		*resulting_elem_data = NULL;
		VERBOSE(stderr, 
			"LL_TaskInstanceGetFirstAdapter = NULL\n");
	...
	case LL_TaskInstanceGetNextAdapter:
		...
		*resulting_elem_data = NULL;
		VERBOSE(stderr,
			"LL_TaskInstanceGetNextAdapter = NULL\n");
		break;
	case LL_TaskInstanceGetFirstAdapterUsage:
		/* LL_element * (AdapterUsage)		*/
		*resulting_elem_data = _build_adapter(slurm_elem);
		if (*resulting_elem_data)
			VERBOSE(stderr, "LL_TaskInstanceGetFirstAdapterUsage\n");
		else
			VERBOSE(stderr, 
				"LL_TaskInstanceGetFirstAdapterUsage = NULL\n");
	...
	case LL_TaskInstanceMachine:
		...
		if (*resulting_elem_data)
			VERBOSE(stderr, "LL_TaskInstanceMachine set\n");
		else
			ERROR(stderr, "LL_TaskInstanceMachine = NULL\n");
		break;
	case LL_TaskInstanceMachineAddress:
		...
		*resulting_char_data = node_data->node_addr;
		...
		VERBOSE(stderr, "LL_TaskInstanceMachineAddress = %s\n",
	...
	case LL_TaskInstanceMachineName:
		...
		*resulting_char_data = node_data->node_name;
		VERBOSE(stderr, "LL_TaskInstanceMachineName = %s\n",
			*resulting_char_data);
		break;
	case LL_TaskInstanceTaskID:
		/*  int * : task id			*/
		*resulting_int_data = taski_data->task_id;
		VERBOSE(stderr, "LL_TaskInstanceTaskID = %d\n",
			*resulting_int_data);
		break;
	default:
		ERROR(stderr, "ERROR: ll_get_data: unsupported "
 ...
 *
 * FIX ME! - Who frees the protocol, mode, and device buffers??
 */	
static void _parse_network_string(char *network_str, char **protocol,
				  char **mode, char **device)
{
	char *orig, *state, *tok, *tmp;
	char *env_protocol, *env_mode, *env_device;

	if ((env_protocol = getenv("MP_MSG_API")) != NULL)
		VERBOSE(stderr, "MP_MSG_API    = \"%s\"\n", env_protocol);
	if ((env_mode =     getenv("MP_EUILIB")) != NULL)
		VERBOSE(stderr, "MP_EUILIB     = \"%s\"\n", env_mode);
	if ((env_device =   getenv("MP_EUIDEVICE")) != NULL)
		VERBOSE(stderr, "MP_EUIDEVICE  = \"%s\"\n", env_device);
	/*
	 * If poe did not provide a network string command, check the poe
	 * environment variables for settings.  Use defaults if an env
	 * variable is not found.
	 *
	 * NOTE:  poe sets all three of these env variables, even if they
	 *        weren't set at the time that poe was exec'ed.
	 */
	if (network_str == NULL) {
		if (!strcmp(getenv("LOADLBATCH"), "yes")) {
			/* Batch mode */
			if (env_protocol != NULL)
				*protocol = strdup(env_protocol);
			else
				*protocol = strdup("mpi");
			VERBOSE(stderr, "Using protocol = \"%s\"\n", *protocol);

			if (env_mode != NULL) {
				*mode = strdup(env_mode);
			} else {
				/*
				 * Maybe this default should be "ip"?
				 * After all, interactive-poe defaults to ip.
				 */
				*mode = strdup("us");
			}
			VERBOSE(stderr, "Using mode     = \"%s\"\n", *mode);

			if (env_device != NULL) {
				*device = strdup(env_device);
				if ((strcmp(*mode, "us") == 0)
				    && (strncmp(*device, "sn", 2) != 0))
					*device = strdup("sn_all");
			} else {
				if (!strcmp(*mode, "us"))
					*device = strdup("sn_all");
				else
			...
			VERBOSE(stderr, "Using device   = \"%s\"\n", *device);

		} else {
			/*
			 * Interactive mode, but no network string sent
			 * by poe, so assume the defaults.
			 */
			*protocol = strdup("not specified");
			*mode = strdup("ip");
			*device = strdup("");
		}
		return;
	...
	if (!orig) {
		ERROR(stderr, "Could not copy network string ");
		goto done;
	}
	...
	*protocol = strdup(tmp + strlen("network."));
	if (!*protocol) {
		ERROR(stderr, "Failed to copy protocol string ");
		return;
	}
	...
	*device = strdup(tok);
	if (!*device) {
		ERROR(stderr, "Failed to copy device string ");
	        goto done;
	}
	...
	*mode = strdup(tok);
	if (!*mode) {
		ERROR(stderr, "Failed to copy mode string ");
		goto done;
	}
...
static slurm_elem_t *_build_adapter(slurm_elem_t *taski_elem)
{
	slurm_elem_t *rc = (slurm_elem_t *)NULL;
	slurm_elem_t *tmp;
	...
	if(!strcmp(adapter_data->device, "sn_all")) {
		...
		if(device) {
			VERBOSE(stderr, "device = %s\n", device);

			device += taski_data->task_id * 5;
			free(adapter_data->device);
			...
			if(adapter_data->device[2] == 'i') {
				...
				ERROR(stderr, "ERROR: don't understand this "
				      "type of adapter %s\n", 
				      adapter_data->device);
				free(adapter_data->protocol);
				free(adapter_data->mode);
			...
			ERROR(stderr, "ERROR: no device returned "
			      "from slurm\n");
		}
	} else if (!strncmp(adapter_data->device,"sn",2)) {
		ERROR(stderr, "ERROR: don't specify the sn adapter\n");
		free(adapter_data->protocol);
		free(adapter_data->mode);
		free(adapter_data->device);
		free(adapter_data);
	...
	if(slurm_jobinfo_ctx_get(
		   jobinfo, FED_JOBINFO_TABLE, &table) < 0) {
		ERROR(stderr, "ERROR: 2 jobinfo_ctx_get RESP: %s\n",
		      slurm_strerror(slurm_get_errno()));
		free(adapter_data->protocol);
		free(adapter_data->mode);
		free(adapter_data->device);
	...
	if(table)
		adapter_data->window = 
			table[taski_data->task_id]->window_id;
