...
extern int ll_get_job(LL_element *mgmt_obj, LL_element **job)
{
	...
	VERBOSE(stderr, "++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	VERBOSE(stderr, "ll_get_job\n");

	/* validate arguments */
	if (mgmt_obj == NULL) {
		ERROR(stderr, "ERROR: mgmt_obj is NULL\n");
		rc = -1;
		goto done;
	}
	...
	if (job == NULL) {
		ERROR(stderr, "ERROR: job is NULL\n");
		rc = -1;
		goto done;
	}
...
static int _build_job_obj(slurm_job_init_t *job_data)
{
	...
	job_desc_msg.job_id = jobid_int;
	job_desc_msg.uid    = getuid();
	if (slurm_confirm_allocation(&job_desc_msg, &alloc_resp_msg) < 0) {
		int err = slurm_get_errno();
		ERROR(stderr, "ERROR: slurm_confirm_allocation: %s\n", 
			slurm_strerror(err));
	...
	if (tasks_char && ((tasks_int = atoi(tasks_char)) > 0))
		job_data->slurm_job_desc->num_tasks = tasks_int;
	else {
		/* one task per node */
		job_data->slurm_job_desc->num_tasks = alloc_resp_msg->node_cnt;
	}
