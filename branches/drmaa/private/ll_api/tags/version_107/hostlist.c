...
\*****************************************************************************/
/* Set TEST_MAIN for testing as stand-alone program */
#define TEST_MAIN 0
...
int main(int ac, char **av)
{
    ...
    if (ac == 1)
	printf("Usage: %s [hostlist]\n\n", av[0]);

    if (!(hl1 = hostlist_create(ac > 1 ? av[1] : NULL)))
        perror("hostlist_create");
#if 0
    /* build hostlist based upon names generated names */
    for (i=0; i<4; i++) {
	int j;
	for (j=0; j<1500; j++) {
	    if (j%20 == 0) continue;
	    sprintf(buf, "purple%4.4d", j);
	    hostlist_push_host(hl1, buf);
	    hostlist_push_host(hl1, buf);
	}
    }
#endif
    ...
    hostlist_ranged_string(hl3, 1024000, buf);
    ...
    hostlist_ranged_string(hl1, 1024000, buf);
    printf("ranged   = `%s'\n", buf);

    iterator_test(buf);

    hostlist_deranged_string(hl1, 1024000, buf);
    printf("deranged = `%s'\n", buf);

    hostset_ranged_string(set, 1024000, buf);
    printf("hostset  = `%s'\n", buf);
