...
static int 	_spawn_task(LL_element *jobmgmtObj, LL_element *step,
			char *executable, LL_element *taskI, int flags);
		

extern int ll_spawn_task(LL_element *jobmgmtObj, LL_element *step,
		char *executable, LL_element *taskI, int flags)
{
	slurm_elem_t *slurm_job_elem = (slurm_elem_t *) jobmgmtObj;
	slurm_elem_t *slurm_step_elem = (slurm_elem_t *) step;
	slurm_elem_t *slurm_taski_elem = (slurm_elem_t *) taskI;
	int rc;

	VERBOSE("++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	VERBOSE("ll_spawn_task\n");
	...
	rc = _fetch_fd(jobmgmtObj, step, executable, taskI, flags);
		...
		char *executable, LL_element *taskI, int flags)
{
	...
	slurm_elem_t *taski_elem = (slurm_elem_t *) taskI;
	...
	if ((step_data->fd_array == NULL) &&
	    ((rc = _spawn_task(jobmgmtObj, step, executable, 
				taskI, flags)) < 0))
		return rc;	/* spawn failure */

	taski_data = (slurm_taski_elem_t *) taski_elem->data;
