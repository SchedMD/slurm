		...
		char **object_filter, enum DataFilter data_filter)
{
	...
	if (query_element == NULL) {
		ERROR("ll_set_request: query_element==NULL\n");
		rc = -1;
		goto done;
	}

	VERBOSE("ll_set_request: elem=%s, type=%s, flags=%s\n",
		...
		char **object_filter, enum DataFilter data_filter)
{
	if (query_flags != QUERY_ALL) {
		ERROR("ll_set_request: flags=%s unsupported\n",
				_query_flag_str(query_flags));
		return -2;
	}
		...
		char **object_filter, enum DataFilter data_filter)
{
	...
	if (slurm_elem->data == NULL) {
		ERROR("ll_set_request: data== NULL\n");
		return -1;
	}
...
static char * _get_job_id(char **object_filter)
{
	...
	if (object_filter == (char **)NULL) {
		ERROR("ll_set_request.c: filter_object is NULL\n");
		return NULL;
	}

	if (object_filter[0] == NULL) {
		ERROR("ll_set_request.c: filter_object is empty\n");
		return NULL;
	}

	if (object_filter[1] != NULL) {
		ERROR("ll_set_request.c: filter_object has too "
			"many  entries\n");
		return NULL;
	}
	...
	if (end_ptr || (job_id <= 0) || (job_id == LONG_MIN) || 
			(job_id == LONG_MAX)) {
		ERROR("ll_set_request.c: filter_object invalid(%s)\n",
			object_filter[0]);
		return NULL;
	}
