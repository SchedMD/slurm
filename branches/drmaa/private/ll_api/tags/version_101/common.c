...
extern int build_step_ctx(slurm_elem_t *job_elem, slurm_elem_t *step_elem)
{
	...
	if (step_data->ctx == NULL) {
		err_code = slurm_get_errno();
		ERROR("slurm_step_ctx_create: %s\n", 
			slurm_strerror(err_code));
		if (err_code == ESLURM_INTERCONNECT_FAILURE) {
			ERROR("  All switch windows may be in use\n");
			ERROR("  Do you have other running jobs steps?\n");
			ERROR("  Run \"squeue -s\" to check\n");
		}
		return -1;
	}
		...
		slurm_job_init_t *slurm_job_init_ptr)
{
	...
	r->num_tasks  = slurm_job_init_ptr->task_cnt;

	/*
	 * The node_list from the slurm job allocation contains only unique
	 * node names, so for an INTERACTIVE_HOSTLIST_SESSION, we need
	 * to go back to the orginal hostlist_t with repeated hostnames.
	 */
	if (poe_session == INTERACTIVE_HOSTLIST_SESSION) {
		r->node_list = slurm_job_init_ptr->slurm_job_desc->req_nodes;
	} else {
		r->node_list = job_resp->node_list;
	}
	VERBOSE("step request node_list = %s\n", r->node_list);
