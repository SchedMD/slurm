...
/* Externally reachable variable with name known by TotalView */
extern char *totalview_jobid;

extern enum SessionType poe_session;

extern FILE *ll_api_stderr;

/* If this is set to true, then we should avoid all federation
   driver functionality */
extern bool switch_none;
...
typedef struct step_elem {
	...
	/* Network parameters */
	char *			protocol;	/* MPI, LAPI, etc. */
	char *			mode;		/* IP or US */
	char *			device;		/* sn_all, sn_single, etc. */
} slurm_step_elem_t;
...
#define INIT_LOGGING()						\
	do {							\
		if (ll_api_stderr == NULL) {			\
			int fd = -1;				\
			fd = dup(STDERR_FILENO);		\
			ll_api_stderr = fdopen(fd, "a");	\
		}						\
	} while(0)
...
#define ERROR(...)							\
	do {								\
		...
		if (num > 2) {						\
			...
			fprintf(ll_api_stderr, "SLURMERROR: " __VA_ARGS__);\
			fflush(ll_api_stderr);				\
		}							\
	} while(0)
...
#define VERBOSE(...)							\
	do {								\
		...
		if (num > 2) {						\
			...
			fprintf(ll_api_stderr, __VA_ARGS__);		\
			fflush(ll_api_stderr);				\
		}							\
	} while(0)
