...
FILE *ll_api_stderr = NULL;

/* If this is set to true, then we should avoid all federation
   driver functionality */
bool switch_none = false;

static slurm_step_ctx_params_t *_step_req_create(
...
extern int build_step_ctx(slurm_elem_t *job_elem, slurm_elem_t *step_elem)
{
	...
	if (!(ctx_params = _step_req_create(job_init_data))) {
		ERROR("Unable to allocate step request message\n");
		return -1;
	}

	step_data->ctx = slurm_step_ctx_create(ctx_params);
	...
	job_init_data->job_alloc_resp->node_list = strdup(ctx_params->node_list);
...
static slurm_step_ctx_params_t *_step_req_create(slurm_job_init_t *
						 slurm_job_init_ptr)
{
	slurm_step_ctx_params_t *r;
	...
	r = malloc(sizeof(slurm_step_ctx_params_t));
	if (r == NULL) {
		ERROR("malloc error");
		return NULL;
	}
	slurm_step_ctx_params_t_init(r);

	r->job_id     = job_resp->job_id;
	r->uid    = (uid_t)job_req->user_id;
	r->node_count = job_resp->node_cnt;
	/* Processor count not relevant to poe */
	r->cpu_count  = job_resp->node_cnt;
	r->task_count = slurm_job_init_ptr->task_cnt;
	VERBOSE("BAR task_count = %d\n", r->task_count);
