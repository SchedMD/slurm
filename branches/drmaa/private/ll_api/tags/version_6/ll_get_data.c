		       ...
		       void *resulting_data_type)
{
	...
	VERBOSE("element=%s, spec=%d\n",
		elem_name(slurm_elem->type), specification);
			       ...
			       void *resulting_data_type)
{
	...
	case LL_TaskInstanceAdapterCount:
		...
		*resulting_int_data = _get_adapter_count(slurm_elem);
		/*
		 * If _get_adapters_count is 0, assume we are in ip mode
	...
	if(!strcmp(step_data->device, "sn_all")
	   || !strcmp(step_data->device, "sn_single")) {
		if(tableinfo) {
			...
			table = tableinfo[adapter_idx].table[taski_data->task_id];
