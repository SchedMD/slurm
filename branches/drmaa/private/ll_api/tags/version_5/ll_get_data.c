 ...
 *
 * FIXME! - Who frees the protocol, mode, and device buffers??
 */	
static void _parse_network_string(char *network_str, char **protocol,
				  char **mode, char **device)
{
 ...
 */
static void _set_network_parameters(slurm_adapter_elem_t *adapter_data,
				    slurm_job_init_t *job_data)
{
	if (!strcmp(getenv("LOADLBATCH"), "yes")) {
		/* Batch mode */
		VERBOSE("poe is in batch mode\n");
		_parse_slurm_network_env(&adapter_data->protocol,
					 &adapter_data->mode,
					 &adapter_data->device);
		 ...
		 */
		VERBOSE("poe is in batch mode, but not network string\n");
		adapter_data->protocol = strdup("not specified");
		adapter_data->mode = strdup("ip");
		adapter_data->device = strdup("");
	}
...
static slurm_elem_t *_build_adapter(slurm_elem_t *taski_elem, int adapter_idx)
{
	...
	_set_network_parameters(adapter_data, job_data);
	...
	if(!strcmp(adapter_data->device, "sn_all")
	   || !strcmp(adapter_data->device, "sn_single")) {
		if(tableinfo) {
...
fail1:
	free(adapter_data->protocol);
	free(adapter_data->mode);
	free(adapter_data->device);
	free(adapter_data);
	free(rc);
	rc = NULL;
