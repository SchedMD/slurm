...
{
	fprintf(stderr, "SLURMINFO: Job %u is pending allocation of resources.\n",
		job_id);
}
...
extern int ll_request(LL_element *jobmgmtObj, LL_element *job)
{
	...
	if (poe_session == INTERACTIVE_HOSTLIST_SESSION) {
		...
		slurm_job_init_ptr->task_cnt = count;
	...
	slurm_job_desc_ptr->immediate = 0;
retry:
	if ((job_alloc_resp = slurm_allocate_resources_blocking(
		     slurm_job_desc_ptr, 0, pending_callback)) < 0) {
