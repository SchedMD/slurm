...
extern int ll_init_job(LL_element **jobmgmtObj)
{
	...
	slurm_job_desc->user_id = (uint32_t)getuid();
	slurm_job_desc->group_id = (uint32_t)getgid();
...
static int _valid_slurm_config(void)
{
	...
	if (strcmp(slurm_config_ptr->switch_type, "switch/federation")) {
		if (strcmp(slurm_config_ptr->switch_type, "switch/none") == 0) {
			VERBOSE("switch/none in use, no federation network support\n");
			switch_none = true;
		} else {
