...
extern void parse_command_line(int argc, char *argv[])
{
	...
	while ((opt_char = getopt_long(argc, argv, 
		...
		case (int)'?':
			...
			break;
		case (int)'a':
			params.primary_slurmctld_failure = true;
			break;
		case (int)'A':
			params.primary_slurmctld_resumed_operation = true;
			break;
		case (int)'b':
			params.primary_slurmctld_resumed_control = true;
			break;
		case (int)'B':
			params.backup_slurmctld_failure = true;
			break;
		case (int)'c':
			params.backup_slurmctld_resumed_operation = true;
			break;
		case (int)'C':
			params.backup_slurmctld_assumed_control = true;
			break;
		case (int)'d':
		...
		case (int)'D':
			...
			break;
		case (int)'e':
			params.primary_slurmctld_acct_buffer_full = true;
			break;
		case (int)'f':
		...
		case (int)'F':
			...
			break;
		case (int)'g':
			params.primary_slurmdbd_failure = true;
			break;
		case (int)'G':
			params.primary_slurmdbd_resumed_operation = true;
			break;
		case (int)'h':
			params.primary_database_failure = true;
			break;
		case (int)'H':
			params.primary_database_resumed_operation = true;
			break;
		case (int)'i':
...
static void _init_options( void )
{
	...
	params.primary_slurmctld_failure = false;
	params.primary_slurmctld_resumed_operation = false;
	params.primary_slurmctld_resumed_control = false;
	params.primary_slurmctld_acct_buffer_full = false;
	params.backup_slurmctld_failure = false;
	params.backup_slurmctld_resumed_operation = false;
	params.backup_slurmctld_assumed_control = false;
	...
	params.primary_slurmdbd_failure = false;
	params.primary_slurmdbd_resumed_operation = false;
	params.primary_database_failure = false;
	params.primary_database_resumed_operation = false;
	params.job_id       = 0;
	params.node_id      = NULL;
	params.offset       = 0;
	params.program      = NULL;
...
static void _print_options( void )
{
	...
	verbose("primary_slurmctld_failure  = %s", \
		params.primary_slurmctld_failure ? "true" : "false");
	verbose("primary_slurmctld_resumed_operation  = %s", \
		params.primary_slurmctld_resumed_operation ? "true" : "false");
	verbose("primary_slurmctld_resumed_control  = %s", \
		params.primary_slurmctld_resumed_control ? "true" : "false");
	verbose("primary_slurmctld_acct_buffer_full  = %s", \
		params.primary_slurmctld_acct_buffer_full ? "true" : "false");
	verbose("backup_slurmctld_failure  = %s", \
		params.backup_slurmctld_failure ? "true" : "false");
	verbose("backup_slurmctld_resumed_operation  = %s", \
		params.backup_slurmctld_resumed_operation ? "true" : "false");
	verbose("backup_slurmctld_assumed_control = %s", \
		params.backup_slurmctld_assumed_control ? "true" : "false");
	verbose("primary_slurmdbd_failure  = %s", \
		params.primary_slurmdbd_failure ? "true" : "false");
	verbose("primary_slurmdbd_resumed_operation  = %s", \
		params.primary_slurmdbd_resumed_operation ? "true" : "false");
	verbose("primary_database_failure  = %s", \
		params.primary_database_failure ? "true" : "false");
	verbose("primary_database_resumed_operation  = %s", \
		params.primary_database_resumed_operation ? "true" : "false");
...
static void _validate_options( void )
{
	...
	&&  ((params.node_down + params.node_drained + params.node_fail +
	      ...
	      params.job_fini  + params.time_limit + params.block_err +
     	      params.primary_slurmctld_failure  +
	      params.primary_slurmctld_resumed_operation  +
	      params.primary_slurmctld_resumed_control  +
	      params.primary_slurmctld_acct_buffer_full  +
 	      params.backup_slurmctld_failure +
	      params.backup_slurmctld_resumed_operation  +
	      params.backup_slurmctld_assumed_control  +
	      params.primary_slurmdbd_failure  +
	      params.primary_slurmdbd_resumed_operation  +
	      params.primary_database_failure  +
	      params.primary_database_resumed_operation) == 0)) {
...
Usage: strigger [--set | --get | --clear] [OPTIONS]\n\
  ...
  -A, --primary_slurmctld_resumed_operation trigger event on primary\n\
        slurmctld resumed operation after failure\n\
  -b, --primary_slurmctld_resumed_control  trigger event on primary slurmctld\n\
                                           resumed control\n\
  -B, --backup_slurmctld_failure  trigger event when backup slurmctld fails\n\
  -c, --backup_slurmctld_resumed_operation  trigger event when backup\n\
        slurmctld resumed operation after failure\n\
  -C, --backup_slurmctld_assumed_control  trigger event when backup slurmctld\n\
                                          assumed control\n\
  -d, --down          trigger event when node goes DOWN\n\
  -D, --drained       trigger event when node becomes DRAINED\n\
  -e, --primary_slurmctld_acct_buffer_full  trigger event when primary\n\
        slurmctld acct buffer full\n\
  ...
  -G, --primary_slurmdbd_resumed_operation  trigger when primary slurmdbd\n\
                                            resumed operation after failure\n\
  -h, --primary_database_failure  trigger when primary database fails\n\
  -H, --primary_database_resumed_operation  trigger when primary database\n\
                                            resumed operation after failure\n\
