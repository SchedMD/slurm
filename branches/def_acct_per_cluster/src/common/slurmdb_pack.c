...
extern void slurmdb_pack_user_cond(void *in, uint16_t rpc_version, Buf buffer)
{
	...
	if (rpc_version >= 8) {
		...
		if (object->assoc_cond && object->assoc_cond->acct_list)
			count = list_count(object->assoc_cond->acct_list);

		pack32(count, buffer);

		if (count && count != NO_VAL) {
			itr = list_iterator_create(
				object->assoc_cond->acct_list);
			while ((tmp_info = list_next(itr))) {
				packstr(tmp_info, buffer);
			}
			list_iterator_destroy(itr);
				    ...
				    Buf buffer)
{
	...
	*object = object_ptr;
	...
	if (rpc_version >= 8) {
		...
		safe_unpack32(&count, buffer);
		if (count != NO_VAL) {
			if (!object_ptr->assoc_cond->acct_list)
				object_ptr->assoc_cond->acct_list =
					list_create(slurm_destroy_char);
			for (i=0; i<count; i++) {
				safe_unpackstr_xmalloc(
					&tmp_info, &uint32_tmp, buffer);
				list_append(object_ptr->assoc_cond->acct_list,
					    tmp_info);
			}
		}
