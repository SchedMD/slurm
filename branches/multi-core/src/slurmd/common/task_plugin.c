...
typedef struct slurmd_task_ops {
	int		(*slurmd_launch_request)	( launch_tasks_request_msg_t *req, uint32_t srun_node_id);
	int		(*slurmd_reserve_resources)	( uint32_t job_id, launch_tasks_request_msg_t *req );
...
extern int slurmd_launch_request( launch_tasks_request_msg_t *req, uint32_t srun_node_id)
{
	...
	return (*(g_task_context->ops.slurmd_launch_request))(req, srun_node_id);
}
