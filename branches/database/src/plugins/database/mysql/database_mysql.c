...
{
#ifdef HAVE_MYSQL
	uint32_t port = slurm_get_database_port();	
	char *host = slurm_get_database_host();	
	char *user = slurm_get_database_user();	
	char *pass = slurm_get_database_pass();	
	...
	if(!(mysql_db = mysql_init(mysql_db)))
		...
		while(!database_init) {
			if(!mysql_real_connect(mysql_db, host, user,
					       pass, NULL, port, NULL, 0)) {
