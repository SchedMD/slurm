...
controller for a cluster with 1000 nodes will occupy on the order of 2 MB 
of memory and performance will be excellent. 
...
SLURM is not a meta-batch system like Globus\footnote{http://www.globus.org/}
or DPCS (Distributed Production Control 
System)\footnote{http://www.llnl.gov/liv\_comp/DPCS/DPCS\_home.html}.  
...
utilities: \srun\, \scancel\, \sinfo\, \squeue\, and \scontrol\,
...
a job is assigned a set of nodes, the user will be able to initiate
...
\slurmd\ reads its configuration from a file: {\tt /etc/slurm.conf}
...
\slurmctld\ daemon or an \srun\ or \scancel\ process. Starting a process may
...
When \slurmctld\ starts, it reads its configuration from a file: 
...
Most \scontrol\ functions can only be executed by user {\tt root} 
or user {\tt SlurmUser} (as defined in the SLURM configuration file).
...
Every set of executing parallel tasks will
have an associated \srun\ process which is managing it. 
...
over 1000 nodes without difficulty over a Gigabit Ethernet. 
We are reviewing other communication 
mechanisms which may offer improved scalability. One possible alternative 
is STORM \footnote{http://www.c3.lanl.gov/~fabrizio/papers/sc02.pdf}. 
...
independent format. We did consider the use of XML style messages, 
...
information.  Only the user {\tt root} or {\tt SlurmUser} (as defined 
in the SLURM configuration file) may modify the SLURM configuration,
...
authd\footnote{http://www.cs.berkeley.edu/~bnc/authd/} and 
...
generate a Mesage Authentication Code (MAC) for the credential.
\munged\ will then use a public key to symmetrically encrypt 
...
upon job initiation. A \slurmd\ may decrypt this credential with the
...
beyond communication encryption and job credentials. In particular, if a
...
the request is {\tt root} or the {\tt SlurmUser} per the SLURM configuration 
file.  The request from such a user may submit a job as any other user. 
...
A user wishes to run a job in batch mode, in which \srun\ will return 
...
The remote \slurmd\ establishes the new environment, execute a SLURM 
prolog program as user {\tt root} (if one is configured), and executes the
...
and begins cleanup. It directs the {\tt slurmd}'s formerly assigned to the
job to run the SLURM epilog program (if so configured). Finally,
...
A user wishes to run the same job in interactive mode, in which \srun\
...
for fault tolerance.  The controller includes the following subsystems:
...
Actual node resources will be checked on \slurmd\ initialization and
periodically
thereafter.  If a node registers with less resources than configured, it
will be placed in {\em DOWN} state and the event will be logged.  Otherwise the
...
System specific scripts can be executed as user root on each allocated 
...
A backup \slurmctld\, if one is configured, will periodically ping
the primary {\tt slurmctld}.  Should the primary \slurmctld\ cease
...
the /etc/slurm.conf file, capture system state, attempt an initial
...
\slurmd\ will accept requests from \srun\ and \slurmctld\ to initiate
...
the {\tt slurmctld}, which will log the event and set node's state to DRAINED.
...
\scancel\ will prematurely terminate queued or running jobs or job steps.
...
through input specification of: SLURM job ID, job step ID, user name, 
partition name, and/or job state. 
If a job ID is supplied, all job steps associated with the job will be
...
all jobs will be reported. \squeue\ also has a variety of sorting and 
output options.

\subsection{sinfo}

\sinfo\ will report the state of SLURM partitions and nodes.  By
...
startup. \srun\ will request resources from the {\tt slurmctld}, which will 
...
SLURM supports the TotalView debugger\footnote{http://www.etnus.com}. 
...
\begin{figure}[htb]
\centerline{\epsfig{file=times.eps}}
...
\item[Munged]   User authentication mechanism
