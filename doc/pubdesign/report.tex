...
\documentclass[10pt,onecolumn]{llncs}
...
scheduling system for Linux clusters of thousands of nodes.  
...
names using a numeric range in the SLURM configuration file 
...
Note the node name syntax with numeric range permits even very large
...
may initiate multiple jobs on the nodes for improved system utilization
...
message broadcast only if the nodes are contiguous.  If a job
...
node and it will be pinged periodically to determine when it resumes 
...
Only the host and port information is referenced by most commands.
...
Our intent is to provide a plugin for use by an external scheduler 
to establish this initial priority. 
A plugin function would also be called at the start of each scheduling 
cycle to modify job or system state as desired.
...
rather than a collection of parallel tasks. 
The job script executes \srun\ commands to initiate the parallel tasks 
...
state save and assumes control.  
...
backup controller before returning an error.
...
The job termination request contains the SLURM job ID.  
...
If a job step ID is supplied, only that job step is effected.
...
\item {\em Update node state}: Update the state of a particular node. Note that 
...
\item {\em Update partition state}: Update the state of a partition node. Note 
that not all state information can be changed in this fashion (e.g. the default 
...
special control sequence typed at the user's terminal. 
Output from the running job is subsequently 
...
computers designated as both primary and backup control machine can 
easily be relocated as needed without loss of the workload by 
changing the configuration file and restarting all SLURM daemons. 
