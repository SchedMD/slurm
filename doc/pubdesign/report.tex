...
\marginpar{Moe - could you update figure~\ref{archdetail} to move
switch manager to controller, add stream copy service to slurmd, and
change job management to process manager in slurmd?  Thanks --JG}  
...
it has at any given time pertains to the currently executing job, if any.
...
Job status includes accounting information which may be required by
an external metabatch system such as DPCS.

\item{\em Process Manager}:  Start, monitor, and clean up after
...
partitions to {\em up}, {\em down}, or {\em drain} status), and
...
\item {\em Scheduler}: Determine the order in which jobs are executed.
When jobs are first submitted, they are placed in the job queue and the 
scheduler is called to perform any reordering of the queue dictated by 
its policy.  The default scheduler implements FIFO with backfill.
The default may be replaced with an external scheduler which implements
other policies.  

\item {\em Job Manager}:  Manage the job queue.
...
starts jobs until a partition manager resource allocation fails.
On allocation failure, the job manager goes back to sleep.

\item {\em Job Status Manager}:
Poll {\tt slurmd}'s on compute nodes that are running jobs.
When the job status manager detects that one or more nodes have completed 
...
time, initially either Kerberos V or ...?
...
The controller calls the scheduler to reorder the batch queue and awakens the 
job manager.
...
When the tasks complete, the {\tt slurmd}'s on the two compute nodes notify
the job status manager, which changes the job status to {\em complete} and 
...
First it ensures that all tasks have completed by telling the {\tt slurmd}'s
to terminate the job if it is still running.
Next it retrieves the stdout/stderr spool files from the stream copy service
of each {\tt slurmd} and merges them into a single report, 
which it stores in the user's {\tt srun} directory.

Finally, the job manager frees resources back to the partition manager and
