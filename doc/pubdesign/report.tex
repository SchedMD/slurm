...
permits a variety of different infrastructures to be easily developed. 
...
variety of ways including requirements options and ranges, potentially
permitting earlier job initiation than otherwise possible.
...
\begin{figure}[tb]
\centerline{\epsfig{file=../figures/arch2.eps,scale=0.20}}
...
configuration file, notifies the controller that it is active, waits
...
When \slurmctld\ starts, it reads the SLURM configuration file.  It also
can read additional state information from a checkpoint file generated
by a previous execution of {\tt slurmctld}.  Full controller state
...
should be used.  For example {\tt AuthType=auth/authd} says to use the
plugin for authd based authentication and {\tt PluginDir=/usr/local/lib}
...
we anticipate using the plugin mechanism to easily permit use of
...
{\em root} and {\tt SlurmUser} (as defined in the SLURM configuration file).
...
\paragraph{\em Communication authentication}

Historically, inter-node authentication has been accomplished via the use
of reserved ports and setuid binaries. In this scheme, daemons check the
...
limited resource and setuid binaries are a possible security concern,
...
and gid from the credential as the authoratative identity of the sender.
...
may then be authoratatively verified on any remote nodes. However, {\tt
...
\paragraph{\em Job authentication}
...
job credential and switch resources. \srun\ then contacts \slurmd 's
...
When \slurmd\ recieves a job termination request from the SLURM
...
Each \slurmd\ forks a copy of itself which is resposible for the job
...
nodes to the partition as they succcessfully complete the epilog.
